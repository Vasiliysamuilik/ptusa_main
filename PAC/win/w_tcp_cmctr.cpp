...
//------------------------------------------------------------------------------
tcp_communicator_win::tcp_communicator_win(const char *name_rus,
                                           const char *name_eng)
    : tcp_communicator(), netOK(0) {
  // Задаем таймаут.
  tv.tv_sec = 0;
...
int tcp_communicator_win::net_init() {
  ...
  if (master_socket < 0) {
    if (G_DEBUG) {
      printf(
          "tcp_communicator_windows:net_init() - can't create master socket: "
          "%s\n",
          WSA_Last_Err_Decode());
    }
    return -4;
  ...
  if (G_DEBUG) {
    printf(
        "tcp_communicator_windows:net_init() - master socket [ %d ] created.\n",
        master_socket);
  }
  ...
  if (res == SOCKET_ERROR) {
    printf(
        "tcp_communicator_windows:net_init() - ошибка  вызова  ioctlsocket: "
        "%s\n",
        WSA_Last_Err_Decode());
  ...
  //     ( char* ) &on, sizeof( on ) ) )
  //     {
  //     printf( "tcp_communicator_windows:net_init() - ошибка  вызова
  //     setsockopt: %s\n",
  //         WSA_Last_Err_Decode() );
  //     closesocket( master_socket );
  //     return -5;
  ...
  //-Привязка сокета.
  int err = bind(master_socket, (struct sockaddr *)&master_socket_state.sin,
  ...
  int val = 1;
  setsockopt(master_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&val,
             sizeof(val));
#ifdef MODBUS
  ...
  if (G_DEBUG) {
    printf(
        "tcp_communicator_windows:net_init() - modbus socket created. Has "
        "number %d\n\r",
        modbus_socket);
  }
  ...
  if (modbus_socket < 0) {
    if (G_DEBUG) {
      perror(
          "tcp_communicator_windows:net_init() - can't create modbus socket");
    }
  ...
  if (err < 0) {
    if (G_DEBUG) {
      printf(
          "tcp_communicator_windows:net_init() - can't bind modbus socket to "
          "port %d : %s\n",
...
int tcp_communicator_win::evaluate() {
  ...
  while (count_cycles < max_cycles) {
      ...
      if (FD_ISSET(sst[i].socket, &rfds)) {
...
#endif
        {
          ...
          if (ioctlsocket(slave_socket, FIONBIO, &mode) == SOCKET_ERROR) {
            printf(
                "Ошибка перевода клиентского сокета в неблокирующий режим: "
                "%s\n",
                WSA_Last_Err_Decode());
    ...
    for (std::map<int, tcp_client *>::iterator it = clients->begin();
      ...
      {
        int err = in_buffer_count = recvtimeout(
...
//------------------------------------------------------------------------------
int tcp_communicator_win::recvtimeout(u_int s, u_char *buf, int len,
