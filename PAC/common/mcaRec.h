/// @file mcaRec.h
/// @brief Реализация класса @ref TRecipeManager для хранения рецептов
/// автоматической моющей станции в энергонезависимой памяти контроллера.
///
...
class TRecipeManager {
 public:
  enum RecipeValues {
    ...
    RV_DELTA_TR,  //разница температур на подаче и возврате (если меньше
                  //заданной, то начинается отсчет времени циркуляции)
    ...
    RV_V_RAB_ML,  //количество литров выше среднего уровня при котором
                  //заканчивается заполнение бачка циркуляции
    RV_V_RET_DEL,  //через сколько литров после начала подачи V2 включать
                   //возвратный насос
    ...
    RV_T_SANITIZER_RINSING,  //Температура ополаскивания после мойки
                             //дезинфецирующим средством
    RV_V_SANITIZER_RINSING,  //Объем ополаскивания после мойки дезинфицирующим
                             //средством
    RV_TM_MAX_TIME_OPORBACHOK,  //Максимальное время от пропадания нижнего
                                //уровня в бачке до завершении операции
                                //опорожнения бачка

    RV_TM_RET_IS_EMPTY,  //время отсутствия протока на возврате для окончания
                         //опорожнения объекта
    RV_V_LL_BOT,  //количество литров от пропадания нижнего уровня в бачке до
    ...
    RV_TM_NO_CONC,  //время появления ошибки "нет концентрации в возвратной
                    //трубе"
    ...
    //-PID2-!>
    RV_TM_MAX_TIME_OPORCIP,  //Максимальное время операции "Опорожнение объекта
                             //CIP"
    ...
    RV_SIGNAL_SANITIZER_PUMP,  //Сигнал out управления дозатором
                               //дезинфицирующего средства
    RV_RESUME_CIP_ON_SIGNAL,  //Автоматическое возобновление мойки при
                              //пропадании ошибки объекта CIP
    RV_SIGNAL_PUMP_CONTROL,  //Сигнал in analog "Управление производительностью
                             //подающего насоса"
    ...
    RV_SIGNAL_PUMP_CONTROL_FEEDBACK,  //Сигнал in analog "Контролируемый уровень
                                      //для насоса подачи"
    RV_SIGNAL_RET_PUMP_SENSOR,  //Сигнал датчика уровня для работы возвратного
                                //насоса
    RV_RET_PUMP_SENSOR_DELAY,  //Задержка выключения возвратного насоса при
    ...
    RV_PIDP_MAX_OUT,  //Верхняя граница пересчета выхода ПИД-регулятора
                      //подогрева
 ...
 private:
  ...
  int WriteMem(unsigned long startaddr, unsigned long length,
               unsigned char* buf, bool is_string = false);
 ...
 public:
  ...
  char* recipeList;
  /// @fn  int TRecipeManager::LoadRecipeToParams(int recipeNo, int
  /// recipeStartPos, int paramsStartPos, int parQuantity, TParams* par)
  /// @brief Загружает указанное число параметров из указанного рецепта с
  /// указанной позиции в указанные параметры
  ...
  int LoadRecipeToParams(int recipeNo, int recipeStartPos, int paramsStartPos,
                         int parQuantity, run_time_params_float* par);
  ...
  /// @param recipeNo Номер рецепта
  /// @return   int 0 - заданный номер рецепта > максимального количества
  /// рецептов
  ...
  /// @fn  int TRecipeManager::setRecipeValue(int recNo, int valueNo, float
  /// newValue)
  ...
  /// @brief Сбрасывает значения параметров указанного рецепта на значения
  /// по-умолчанию
  /// @param recipeNo Номер рецепта
  /// @return   int 0 - ошибка !0 - ОК
  ...
  /// @brief Обработка рецептов, периодически сохраняет текущее имя рецепта в
  /// энергонезависимую память и формирует список рецептов
  /// @return   void
  void EvalRecipe();
  ...
  /// @param lineNo номер линии мойки, начинается с 0. От него зависит
  /// расположение рецептов в памяти
  /// @return
