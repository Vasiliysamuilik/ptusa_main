...
class action {
 public:
  ...
  virtual void set_params(const saved_params_float* new_par) { par = new_par; }
  ...
  virtual int set_bool_property(const char* prop_name, bool value) { return 0; }
  ...
  enum CONSTANTS { MAIN_GROUP = 0, MAIN_SUBGROUP = 0 };
...
class open_seat_action : public action {
 ...
 private:
  ...
  u_int_4 wash_time_upper;  ///< Время промывки верхних седел текущей группы
                            ///< клапанов.
  u_int_4 wash_time_lower;  ///< Время промывки нижних седел текущей группы
                            ///< клапанов.
  ...
  void switch_off_group(std::vector<device*> group, bool is_check = false);
...
class wash_action : public action {
 public:
  wash_action()
      : action("Устройства DI's DO's DEV's R_DEV's AI", G_SUBGROUPS_CNT) {}
...
//-----------------------------------------------------------------------------
/// @brief Содержит информацию об устройствах, которые входят в шаг
...
class operation_state {
 public:
  ...
  void reset_eval_time() { dx_step_time = 0; }
...
#pragma region Новая функциональность по состояниям операции.
  ...
  static constexpr const std::array<const char* const, STATES_MAX>
      en_state_str = {"OFF",       "RUN",      "PAUSE",     "STOP",
                      "",          "",         "",          "",
                      "",          "",         "STARTING",  "PAUSING",
                      "UNPAUSING", "STOPPING", "COMPLETING"};
...
#pragma region Загрузка описания.
  ...
  operation_state* operator[](int idx) {
    if (idx < static_cast<int>(state_idx::STATES_MAX)) {
      ...
      if (G_DEBUG) {
        printf(
            "Error operation_state* operation::operator[] "
...
class operation_manager {
 public:
  ...
  operation* add_mode(const char* name) { return add_operation(name); }
  ...
  void off_mode(int mode) const { owner->set_mode(mode, 0); }

  float get_step_param(u_int idx) const { return owner->get_step_param(idx); }

  const saved_params_float* get_params() const { return owner->get_params(); }
