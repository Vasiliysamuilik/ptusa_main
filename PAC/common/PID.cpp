...
float PID::eval(float currentValue, int deltaSign) {
  ...
  float ek =
      deltaSign * MAX_OUT_VALUE * (set_value - currentValue) / (dmax - dmin);
  ...
  if (get_delta_millisec(last_time) > dt * MSEC_IN_SEC) {
    ...
    //-Зона разгона.
    unsigned long delta_time = get_delta_millisec(start_time);
...
//-----------------------------------------------------------------------------
void PID::set(float new_out) { set_value = new_out; }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void PID::save_param() { par->save_all(); }
//-----------------------------------------------------------------------------
float PID::get_value() { return out_value; }
//-----------------------------------------------------------------------------
void PID::reset() {
  uk_1 = 0;
  ek_1 = 0;
...
//-----------------------------------------------------------------------------
float PID::get_assignment() { return set_value; }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
const char* PID::get_name_in_Lua() const { return get_name(); }
//-----------------------------------------------------------------------------
int PID::get_state() { return static_cast<int>(state); }
//-----------------------------------------------------------------------------
...
int PID::save_device(char* buff) {
  ...
  if (is_old_style) {
    ...
    answer_size += sprintf(buff + answer_size, "%s = { %.2f, %.2f }\n",
...
//-----------------------------------------------------------------------------
device* PID::get_actuator() const { return actuator; }
//-----------------------------------------------------------------------------
