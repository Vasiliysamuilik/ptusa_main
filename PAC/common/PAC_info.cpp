...
void PAC_info::eval() {
  ...
  if (get_delta_millisec(last_check_time) > 1000) {
    ...
    sprintf(up_time_str, "%lu дн. %02lu:%02lu:%02lu", (u_long)up_days,
...
int PAC_info::save_device(char* buff) {
  ...
  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tRESET_BY={},\n",
                           reset_type)
              .size;
  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tUP_DAYS={},\n", up_days)
          .size;
  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tUP_HOURS={},\n", up_hours)
          .size;
  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tUP_MINS={},\n", up_mins)
          .size;
  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tUP_SECS={},\n", up_secs)
          .size;
  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tUP_TIME=\"{}\",\n",
                           up_time_str)
              .size;

  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE,
  ...
  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tP_V_OFF_DELAY_TIME={},\n",
  ...
  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tCMD={},\n", cmd).size;
  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tCMD_ANSWER=\"{}\",\n",
                           cmd_answer)
              .size;

  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tVERSION=\"{}\",\n",
                           PRODUCT_VERSION_FULL_STR)
              .size;

  size +=
      fmt::format_to_n(buff + size, MAX_COPY_SIZE, "\tNODEENABLED = \n\t{{\n\t")
          .size;
  ...
  size += fmt::format_to_n(buff + size, MAX_COPY_SIZE,
                           ...
                           par[P_IS_OPC_UA_SERVER_CONTROL])
              .size;
...
//-----------------------------------------------------------------------------
void PAC_info::print() const { printf("PAC_info\n"); }
//-----------------------------------------------------------------------------
  ...
  if (strcmp(prop, "CMD") == 0) {
    switch ((COMMANDS)(int)val) {
      ...
      case RELOAD_RESTRICTIONS: {
        ...
        cmd = G_LUA_MANAGER->reload_script(SCRIPT_N, "restrictions", cmd_answer,
                                           sizeof(cmd_answer));
        return cmd;
      }
  ...
  if (strcmp(prop, "P_IS_OPC_UA_SERVER_ACTIVE") == 0) {
    ...
    if (val == 0 && prev_val == 1) {
      ...
      if (retval != UA_STATUSCODE_GOOD) {
        ...
        auto r =
            fmt::format_to_n(cmd_answer, sizeof(cmd_answer) - 1, G_LOG->msg);
        cmd_answer[r.size] = '\0';
        return 1;
      }
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
PAC_info* G_PAC_INFO() { return PAC_info::get_instance(); }
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
