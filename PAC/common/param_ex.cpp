...
//-----------------------------------------------------------------------------
void params_manager::final_init(
    int auto_init_params /*= 1*/, int auto_init_work_params /*= 1*/,
    void (*custom_init_params_function)() /*= 0 */) {
  sprintf(G_LOG->msg, "Total memory used: %u of %u bytes[ %.2f%c ].", last_idx,
          C_TOTAL_PARAMS_SIZE, 100. * last_idx / C_TOTAL_PARAMS_SIZE, '%');
...
void params_manager::reset_to_default(void (*custom_init_params_function)(),
                                      int auto_init_params,
                                      int auto_init_work_params) {
  memset(params, 0, C_TOTAL_PARAMS_SIZE);
...
char* params_manager::get_params_data(int size, int& start_pos) {
  ...
  if (last_idx + size > params_mem->get_size()) {
    if (G_DEBUG) {
      printf(
          "params_manager::get_params_data() - is not enough memory ( %d + %d "
          "< %d ) !\n",
...
int params_manager::restore_params_from_server_backup(char* backup_str) {
  ...
  if (false == is_init) {
    const char* extra_cmd =
        ...
        "    cmd = "
        "'sys.'..par_info.object..':set_param('..par_info.par_id..','..\n"
        "        ( index - 1 )..','..value..')'\n"
        "    assert( loadstring( cmd ) )()\n"
        "    end\n"
        "end\n";

    lua_manager::get_instance()->exec_Lua_str(
        extra_cmd,
  ...
  int res = lua_manager::get_instance()->exec_Lua_str(
      backup_str, "params_manager::restore_params_from_server_backup ");
...
int params_test::make_test() {
  ...
  if (test1[0] != 5120 || test1[1] != 120 || test1[2] != 130 ||
      test[0] != 512 || test[1] != 12 || test[2] != 13) {
    if (G_DEBUG) {
