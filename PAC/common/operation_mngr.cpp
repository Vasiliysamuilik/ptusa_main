...
const std::array<const char* const, operation::STATES_MAX>
    operation::en_state_str;
//-----------------------------------------------------------------------------
operation::operation(const char* name, operation_manager* owner, int n)
    : name(name), owner(owner), operation_num(n), stub("заглушка", owner, -1) {
...
//-----------------------------------------------------------------------------
operation::state_idx operation::get_state() const { return current_state; }
//-----------------------------------------------------------------------------
int operation::pause() {
...
//-----------------------------------------------------------------------------
int operation::start() { return start(run_step); }
//-----------------------------------------------------------------------------
int operation::start(int new_run_step) {
  switch (current_state) {
    case IDLE:
      states[IDLE]->finalize();
      if (states[STARTING]->is_empty())
        current_state = RUN;
      else
...
//-----------------------------------------------------------------------------
int operation::check_devices_on_run_state(char* err_dev_name,
...
int operation::process_auto_switch_on() {
  ...
  if (is_first_goto_next_state) {
    ...
    if (get_delta_millisec(start_wait) > wt) {
      unit->set_err_msg("автовключение по запросу отключено", operation_num, 0,
                        ERR);
      was_fail = true;
    }
  }
...
int operation::process_new_state_from_run(int next_state) {
  ...
  switch (static_cast<state_idx>(next_state)) {
    case state_idx::STOP:
      ...
      unit->set_err_msg("автоотключение по запросу", operation_num, 0,
                        tech_object::ERR_MSG_TYPES::ERR_DURING_WORK);
      unit->set_mode(operation_num, state_idx::STOP);
      break;
    ...
    case state_idx::PAUSE:
      ...
      unit->set_err_msg("пауза по запросу", operation_num, 0,
                        tech_object::ERR_MSG_TYPES::ERR_TO_FAIL_STATE);
      break;
...
//-----------------------------------------------------------------------------
int operation::default_process_new_state(state_idx next_state,
                                         state_idx def_state) {
  auto unit = owner->owner;
...
//-----------------------------------------------------------------------------
int operation::get_run_step() const { return run_step; }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
const char* operation::get_name() const { return name.c_str(); }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void operation::to_step(unsigned int new_step,
                        unsigned long cooperative_time /*= 0 */) {
  if (current_state >= 0 && current_state < STATES_MAX) {
...
step* operation::add_step(const char* step_name, int next_step_n /*= -1 */,
                          ...
                          int step_max_duration_par_n /*= -1 */,
                          state_idx s_idx /*= RUN */) {
  if (s_idx >= IDLE && s_idx < STATES_MAX) {
    return states[s_idx]->add_step(step_name, next_step_n, step_duration_par_n,
      ...
      for (auto dev : subgroup) {
        if (dev->get_state() < 0) {
          ...
          if (!(par & base_error::P_IS_SUPPRESS)) {
            auto res =
                fmt::format_to_n(out, free_size, "'{}', ", dev->get_name());
            free_size -= res.size;
...
//-----------------------------------------------------------------------------
void action::clear_dev() { devices.clear(); }
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
delay_on_action::delay_on_action()
    : action("Включать с задержкой"), start_time(get_millisec()) {}
//-----------------------------------------------------------------------------
void delay_on_action::init() { start_time = get_millisec(); }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
delay_off_action::delay_off_action()
    : action("Выключать с задержкой"), start_time(get_millisec()) {}
//-----------------------------------------------------------------------------
void delay_off_action::init() { start_time = get_millisec(); }
//-----------------------------------------------------------------------------
  ...
  *reason = 0;
  ...
  for (auto d : devs) {
    if (!d->is_active()) {
      ...
      auto out = fmt::format_to_n(reason, max_len - 1, f_str, d->get_name(),
                                  d->get_description());
      *out.out = '\0';
      return 1;
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
checked_devices_action::checked_devices_action()
    : action("Проверяемые устройства") {}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
step::step(std::string name, operation_state* owner, bool is_mode /*= false */)
...
//-----------------------------------------------------------------------------
void step::set_start_time(u_int_4 start_time) { this->start_time = start_time; }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void step::set_dx_time(u_int_4 dx_time) { this->dx_time = dx_time; }
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
int AI_AO_action::check(char* reason, unsigned int max_len) const {
  ...
  for (u_int i = 0; i < devs.size(); i++) {
    ...
    if (do_device->get_type() != device::DT_AI &&
      ...
      auto out =
          fmt::format_to_n(reason, max_len - 1, format_str, name.c_str(),
...
void open_seat_action::init() {
  ...
  if (!is_mode) {
    u_int_4 wait_time =
        owner->get_active_step_set_time() / ((u_int_4)groups_cnt + 1);
    if (wait_time > 0) {
      this->wait_time = wait_time;
    }
  }
  ...
  wait_time -=
      (wash_time_upper > wash_time_lower ? wash_time_upper : wash_time_lower) /
      2;
...
void open_seat_action::evaluate() {
  if (wash_lower_seat_devices.empty() && wash_upper_seat_devices.empty())
    return;
...
void open_seat_action::add_dev(device* dev, u_int group, u_int seat_type) {
  auto seat_group = seat_type == valve::V_LOWER_SEAT ? &wash_lower_seat_devices
                                                     : &wash_upper_seat_devices;
...
bool open_seat_action::is_empty() const {
  ...
  if (wash_upper_seat_devices.size() > 0 &&
      wash_upper_seat_devices[0].empty() &&
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
jump_if_action::jump_if_action(const char* name) : action(name, G_GROUPS_CNT) {}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool jump_if_action::check(const std::vector<device*>& checked_devices,
  ...
  for (auto dev : checked_devices) {
    ...
    if (type == device::DT_V) {
      ...
      if (check_is_opened && !v->is_opened() ||
          !check_is_opened && !v->is_closed())
        return false;
    } else {
      if (check_is_opened && !dev->is_active() ||
          !check_is_opened && dev->is_active())
        return false;
    }
  }
  return true;
...
int jump_if_action::set_int_property(const char* name, size_t idx, int value) {
      ...
      strcmp(name, "next_state_n") == 0)  //Для перехода к новому состоянию.
  {
    ...
    if (G_DEBUG) {
      G_LOG->warning("\"%s\" unknown property \"%s\"", this->name.c_str(),
                     name);
    }
  }
...
//-----------------------------------------------------------------------------
void jump_if_action::finalize() { return; }
//-----------------------------------------------------------------------------
void jump_if_action::print(const char* prefix, bool new_line) const {
  action::print(prefix, false);
  printf(" { ");
  std::for_each(next_n.begin(), next_n.end(),
                [&](int const& n) { printf("%d ", n); });
  printf("}");
  if (new_line) {
    printf("\n");
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
enable_step_by_signal::enable_step_by_signal()
    : action("Включить шаг по сигналам"){};
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
operation_state::operation_state(const char* name, operation_manager* owner,
                                 int n)
    : name(name),
...
//-----------------------------------------------------------------------------
step* operation_state::add_step(const char* step_name,
                                int next_step_n /*= -1 */,
                                int step_duration_par_n /*= -1 */,
                                int step_max_duration_par_n /*= -1 */) {
      ...
      if (-1 == active_step_next_step_n) {
        if (n > 0) {
          ...
          std::snprintf(buff, MAX_BUFF_SIZE,
                        "вышло время (%u сек) последнего шага (\'%s\')", time,
...
step* operation_state::operator[](int idx) {
  ...
  if (G_DEBUG) {
    printf(
        "Error operation_state::step& operator[] ( int idx ) - idx %d > count "
        "%zd.\n",
        idx, steps.size());
  }
...
void operation_state::to_step(u_int new_step, u_long cooperative_time) {
  ...
  if (G_DEBUG) {
    fmt::print(
        R"({}"{}" operation {} "{}" to_step() -> {}, step time {} ms, next step {})",
        owner->owner->get_prefix(), owner->owner->get_name(), n, name.c_str(),
        new_step, active_step_time, active_step_next_step_n);
...
//-----------------------------------------------------------------------------
int operation_state::check_max_step_time(char* err_dev_name,
                                         unsigned int str_len) {
  if (step_max_duration_par_ns
          .empty())  // Не заданы параметры максимального времени.
    return 0;
...
//-----------------------------------------------------------------------------
int operation_state::check_steps_params(char* err_dev_name,
                                        unsigned int str_len) {
...
//-----------------------------------------------------------------------------
u_int operation_state::active_step() const { return active_step_n + 1; }
//-----------------------------------------------------------------------------
u_int operation_state::steps_count() const { return (u_int)steps.size(); }
//-----------------------------------------------------------------------------
const char* operation_state::get_name() const { return name.c_str(); }
//-----------------------------------------------------------------------------
...
int operation_state::on_extra_step(int step_idx) {
  ...
  if (std::find(active_steps.begin(), active_steps.end(), step_idx) ==
      active_steps.end()) {
    ...
    if (steps[step_idx - 1]->check(err_str, sizeof(err_str)) == 0) {
      ...
      if (G_DEBUG) {
        ...
        printf("%s\"%s\" operation %d \"%s\" on_extra_step() -> %d.\n",
               owner->owner->get_prefix(), owner->owner->get_name(), n,
               name.c_str(), step_idx);
    ...
    G_LOG->notice(
        "operation_state:on_extra_step(...) - step (%d) is already extra on",
        step_idx);
    return 0;
  }
...
int operation_state::off_extra_step(int step_idx) {
  if ((size_t)step_idx > steps.size()) {
    G_LOG->notice(
        "operation_state:off_extra_step(...) - step (%d) > size (%zu)",
        step_idx, steps.size());
    return 0;
  }
...
int operation_state::switch_active_extra_step(int off_step, int on_step) {
  if ((size_t)on_step > steps.size()) {
    G_LOG->notice(
        "operation_state:switch_active_step(...) - step (%d) > size (%zu)",
        on_step, steps.size());
    return 1;
  }
  ...
  if (1 + active_step_n == on_step) {
    G_LOG->notice(
        "operation_state:switch_active_step(...) - step (%d) is active",
        on_step);
    return 0;
  }
  ...
  if (res != active_steps.end()) {
    ...
    if (G_DEBUG) {
      SetColor(YELLOW);
      printf(
          "%s\"%s\" operation %d \"%s\" switch_active_extra_step() %d -> %d.\n",
          owner->owner->get_prefix(), owner->owner->get_name(), n, name.c_str(),
          off_step, on_step);
    ...
    *res = on_step;
    ...
    G_LOG->warning(
        "operation_state:switch_active_step(...) - step (%d) not found",
        off_step);
  }
...
operation* operation_manager::operator[](unsigned int idx) {
  ...
  if (G_DEBUG) {
    printf(
        "Error operation_manager::operator[] idx %d > operations count %zd.\n",
        idx, operations.size());
  }
...
//-----------------------------------------------------------------------------
operation_manager::operation_manager(i_tech_object* owner) : owner(owner) {}
//-----------------------------------------------------------------------------
