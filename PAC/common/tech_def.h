...
///
class tech_object : public i_tech_object,
                    public i_Lua_save_device,
 ...
 public:
  ...
  /// @param par_float_count          - количество сохраняемых параметров типа
  /// float.
  /// @param runtime_par_float_count  - количество рабочих параметров типа
  /// float.
  /// @param par_uint_count           - количество сохраняемых параметров типа
  /// uint.
  ...
  tech_object(const char* name, u_int number, u_int type, const char* name_Lua,
              u_int states_count, u_int timers_count, u_int par_float_count,
  ...
  u_int get_modes_count() const { return operations_count; }
...
#pragma region Работа с ошибками.
  ...
  std::vector<err_info*>& get_errors() { return errors; }

  int get_object_type() { return type; }

  /// @brief Наличие активных событий.
  ///
  bool is_any_message() const { return !errors.empty(); }
...
#pragma endregion
  ...
  const saved_params_float* get_params() const { return &par_float; }

  operation_manager* get_modes_manager() { return operations_manager; }
...
class tech_object_manager {
 public:
  ...
  int get_object_with_active_mode(u_int mode, u_int start_idx, u_int end_idx);
