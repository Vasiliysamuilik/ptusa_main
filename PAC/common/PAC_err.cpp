...
auto_smart_ptr<PAC_critical_errors_manager>
    PAC_critical_errors_manager::instance;
//-----------------------------------------------------------------------------
PAC_critical_errors_manager::PAC_critical_errors_manager() : errors_id(0) {
#ifndef __BORLANDC__
  errors.clear();
#endif  // __BORLANDC__
}
  ...
  for (u_int i = 0; i < errors.size(); i++) {
    ...
    res += sprintf(str + res, "\tdescription = \"%s\",\n",
                   get_alarm_descr((ALARM_CLASS)errors[i].err_class,
                                   (ALARM_SUBCLASS)errors[i].err_sub_class,
                                   errors[i].param, true));

    res += sprintf(str + res, "\t%s\n", "type = AT_SPECIAL,");
    res +=
        sprintf(str + res, "\t%s%s%s\n", "group = '", get_alarm_group(), "',");
    res += sprintf(str + res, "\t%s%d%s\n", "priority = ", ALARM_CLASS_PRIORITY,
                   ",");
    res += sprintf(str + res, "\t%s\n", "state = AS_ALARM,");
...
//-----------------------------------------------------------------------------
const char* PAC_critical_errors_manager::get_alarm_descr(
    ALARM_CLASS err_class, ALARM_SUBCLASS err_sub_class, int par, bool is_set) {
  static char tmp[200] = "";
  sprintf(tmp, "%d-%d-%d : ", (int)err_class, (int)err_sub_class, par);
  ...
  switch (err_class) {
    ...
    case AC_SERVICE:
      switch (err_sub_class) {
        case AS_IO_COUPLER:
          sprintf(tmp + strlen(tmp), "Узел ввода/вывода '%s' ('%s') - ",
    ...
    case AC_NO_CONNECTION:
      ...
      switch (err_sub_class) {
        case AS_IO_COUPLER:
          sprintf(tmp + strlen(tmp), "узлом I/O '%s' ('%s', '%s')",
                  G_IO_MANAGER()->get_node(par - 1)->name,
                  G_IO_MANAGER()->get_node(par - 1)->ip_address,
    ...
    case AC_NET:
      ...
      switch (par) {
        case 0:
          sprintf(tmp + strlen(tmp), "master : ");
          break;

        case 1:
          sprintf(tmp + strlen(tmp), "modbus : ");
          break;
      ...
      switch (err_sub_class) {
        case AS_SOCKET_F:
          sprintf(tmp + strlen(tmp), "calling function socket(...) : ");
          break;

        case AS_BIND_F:
          sprintf(tmp + strlen(tmp), "calling function bind(...) : ");
          break;

        case AS_SETSOCKOPT_F:
          sprintf(tmp + strlen(tmp), "calling function setsockopt(...) : ");
          break;

        case AS_LISTEN_F:
          sprintf(tmp + strlen(tmp), "calling function listen(...) : ");
          break;
...
PAC_critical_errors_manager::critical_error::critical_error(int err_class,
                                                            u_int err_sub_class,
                                                            u_int param)
    : err_class(err_class), err_sub_class(err_sub_class), param(param) {}
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
