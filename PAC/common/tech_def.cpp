...
tech_object::tech_object(const char* new_name, u_int number, u_int type,
                         const char* name_Lua, u_int operations_count,
    ...
    : par_float(saved_params_float(par_float_count, this)),
      ...
      rt_par_uint(
          run_time_params_u_int_4(runtime_par_uint_count, "RT_PAR_UI", this)),
      timers(timers_count),
      number(number),
      type(type),
      cmd(0),
  ...
  u_int state_size_in_int4 =
      operations_count / 32;  // Размер состояния в double word.
  ...
  const int MAX_LENGTH =
      strlen(new_name) + strlen(sign_str) + std::to_string(number).length() + 1;
...
int tech_object::set_mode(u_int operation_n, int newm) {
  ...
  if (G_DEBUG) {
    SetColor(GREEN);
    printf(
        "%sBEGIN \"%s %d\" (%s) set operation №%u (\"%s\") --> %s.\n",
        ...
        if (get_mode(operation_n) == newm) {
          if (newm == 1 && op->get_state() != operation::RUN) {
            ...
            if ((res = lua_check_on_start(operation_n)) == 0) {
              auto run_step = lua_get_run_step_after_pause(operation_n);
              op->start(run_step);
          ...
          if (newm == 0)  // Off mode.
          {
            ...
            res = op->check_devices_on_run_state(res_str + len,
                                                 ERR_STR_SIZE - len);
  ...
  if (G_DEBUG) {
    ...
    for (u_int i = 0; i < state.size(); i++) {
      printf("%sstate[ %d ] = %u (", white_spaces, i, state[i]);
...
int tech_object::lua_exec_cmd(u_int cmd) {
  ...
  return lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "exec_cmd", cmd, "int tech_object::lua_exec_cmd( u_int cmd )");
}
//-----------------------------------------------------------------------------
int tech_object::lua_check_function(const char* function_name,
                                    const char* comment, u_int mode,
                                    bool show_error) {
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    return lua_manager::get_instance()->int_2_exec_lua_method(
        name_Lua, function_name, mode, show_error ? 1 : 0, comment);
  }
...
int tech_object::lua_get_run_step_after_pause(u_int mode) const {
  ...
  if (G_LUA_MANAGER->is_exist_lua_function(name_Lua, function_name)) {
    return G_LUA_MANAGER->int_exec_lua_method(
        name_Lua, function_name, mode,
...
int tech_object::lua_check_on_mode(u_int mode, bool show_error) {
        ...
        if (lua_istable(L, -1)) {
          ...
          if (lua_istable(L, -1)) {
            for (u_int k = 1; k <= G_TECH_OBJECT_MNGR()->get_count(); k++) {
              ...
              if (lua_istable(L, -1)) {
                tech_object* t_obj =
                    G_TECH_OBJECT_MNGR()->get_tech_objects(k - 1);
                for (u_int i = 1; i <= t_obj->get_modes_count(); i++) {
                  lua_rawgeti(L, -1, i);  //Номер ограничения.
                  if (lua_isnumber(L, -1)) {
                    //Есть ограничение, проверяем на активность операции.
                    if (t_obj->get_mode(i)) {
                      if (show_error) {
                        snprintf(err_msg, sizeof(err_msg),
                                 "уже включена операция %.1d \'%s\' для "
                                 "объекта \'%s %d\'",
                                 i,
  ...
  if (G_LUA_MANAGER->is_exist_lua_function(name_Lua, old_function_name)) {
    return lua_manager::get_instance()->int_2_exec_lua_method(
        name_Lua, old_function_name, mode, show_error ? 1 : 0,
  ...
  if (G_LUA_MANAGER->is_exist_lua_function(name_Lua, new_function_name)) {
    return lua_manager::get_instance()->int_2_exec_lua_method(
        name_Lua, new_function_name, mode, show_error ? 1 : 0,
        "int tech_object::lua_check_on_mode( u_int mode )");
  }
...
void tech_object::lua_init_mode(u_int mode) {
  lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "init_mode", mode,
      "void tech_object::lua_init_mode( u_int mode )");
...
int tech_object::lua_init_params() {
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->int_exec_lua_method(
        name_Lua, "init_params_uint", 0, "int tech_object::lua_init_params()");
  }
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->int_exec_lua_method(
        name_Lua, "init_params_float", 0, "int tech_object::lua_init_params()");
  }
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->int_exec_lua_method(
        name_Lua, "init_rt_params_uint", 0,
        "int tech_object::lua_init_params()");
  }
  //Удаляем "init_params_uint" со стека.
  lua_remove(lua_manager::get_instance()->get_Lua(), -1);
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->int_exec_lua_method(
        name_Lua, "init_rt_params_float", 0,
        "int tech_object::lua_init_params()");
  }
  //Удаляем "init_params_float" со стека.
  lua_remove(lua_manager::get_instance()->get_Lua(), -1);
...
int tech_object::lua_init_runtime_params() {
  ...
  return lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "init_runtime_params", 0,
      "int tech_object::lua_init_runtime_params()");
}
//-----------------------------------------------------------------------------
int tech_object::lua_on_pause(u_int mode) {
  lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "on_pause", mode,
      "int tech_object::lua_on_pause( u_int mode )");
  return 0;
}
//-----------------------------------------------------------------------------
int tech_object::lua_on_stop(u_int mode) {
  lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "on_stop", mode, "int tech_object::lua_on_stop( u_int mode )");
  return 0;
}
//-----------------------------------------------------------------------------
int tech_object::lua_on_start(u_int mode) {
  lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "on_start", mode,
      "int tech_object::lua_on_start( u_int mode )");
  return 0;
}
//-----------------------------------------------------------------------------
int tech_object::save_device(char* buff) {
  ...
  res += fmt::format_to_n(buff + res, MAX_COPY_SIZE, "\tACTIVE_STEP_TIME={},\n",
                          duration)
             .size;
...
int tech_object::set_cmd(const char* prop, u_int idx, double val) {
  ...
  if (G_DEBUG) {
    printf(
        "Eror tech_object::set_cmd(...), prop = \"%s\", idx = %u, val = %f\n",
        prop, idx, val);
  }
...
int tech_object::is_check_mode(int mode) const {
  int res = lua_manager::get_instance()->int_exec_lua_method(
      name_Lua, "is_check_mode", mode,
      "int tech_object::is_check_mode( u_int mode )");
...
int tech_object::set_err_msg(const char* err_msg, int mode, int new_mode,
  ...
  switch (type) {
    case ERR_CANT_ON:
      snprintf(new_err->msg, sizeof(new_err->msg),
               "\'%s %d\' - не включена операция %.1d \'%s\' - %s.", name,
               number, mode, (*operations_manager)[mode]->get_name(), err_msg);
      break;
    ...
    case ERR_OFF:
      snprintf(new_err->msg, sizeof(new_err->msg),
               "\'%s %d\' - отключена операция %.1d \'%s\' - %s.", name, number,
               mode, (*operations_manager)[mode]->get_name(), err_msg);
      break;

    case ERR_OFF_AND_ON:
      snprintf(new_err->msg, sizeof(new_err->msg),
               "\'%s %d\' - переход от %.1d \'%s\' к %.1d \'%s\'.", name,
               number, mode, (*operations_manager)[mode]->get_name(), new_mode,
               (*operations_manager)[new_mode]->get_name());

      if (strcmp(err_msg, "") != 0) {
        snprintf(new_err->msg + strlen(new_err->msg) - 1,
                 sizeof(new_err->msg) - strlen(new_err->msg) - 1, " - %s.",
                 err_msg);
      }
      break;
    ...
    case ERR_DURING_WORK:
      if (mode > 0) {
        snprintf(new_err->msg, sizeof(new_err->msg),
                 "\'%s %d\' - операция %.1d \'%s\' - %s.", name, number, mode,
                 (*operations_manager)[mode]->get_name(), err_msg);
      } else {
        snprintf(new_err->msg, sizeof(new_err->msg), "\'%s %d\' - %s.", name,
                 number, err_msg);
      }
    ...
    case ERR_TO_FAIL_STATE:
      snprintf(new_err->msg, sizeof(new_err->msg),
               "\'%s %d\' - авария операции %.1d \'%s\' - %s.", name, number,
               mode, (*operations_manager)[mode]->get_name(), err_msg);
      break;
  ...
  if (errors.size() < E_MAX_ERRORS_SIZE) {
    ...
    if (G_DEBUG) {
      printf("Error: max errors count (%d) is reached.", E_MAX_ERRORS_SIZE);
    }
    delete new_err;
    new_err = nullptr;
  }
...
void tech_object::print() const {
  ...
  printf("par_float[%d] par_uint[%d] rt_par_float[%d] rt_par_uint[%d]\n",
         par_float.get_count(), par_uint.get_count(), rt_par_float.get_count(),
         rt_par_uint.get_count());
  printf("timers[%d]\n", timers.get_count());
...
bool tech_object::is_any_important_mode() {
  ...
  if (has_Lua_impl == 2) {
    int res = lua_manager::get_instance()->int_no_param_exec_lua_method(
        name_Lua, "is_any_important_mode",
...
int tech_object::check_operation_on(u_int operation_n, bool show_error) {
  ...
  if (res && is_check_mode(operation_n) == 1) {
    ...
    if ((res = lua_check_on_mode(operation_n, show_error)) !=
        0)  // Check if possible.
    {
      res += 100;
    }
...
//-----------------------------------------------------------------------------
void tech_object::set_serial_idx(u_int idx) { serial_idx = idx; }
//-----------------------------------------------------------------------------
...
int tech_object_manager::init_params() {
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->void_exec_lua_method(
        "", "init_params", "int tech_object_manager::init_params()");
  }
...
int tech_object_manager::get_object_with_active_mode(u_int mode,
                                                     u_int start_idx,
                                                     u_int end_idx) {
  for (u_int i = start_idx; i <= end_idx && i < tech_objects.size(); i++) {
...
//-----------------------------------------------------------------------------
u_int tech_object_manager::get_count() const { return tech_objects.size(); }
//-----------------------------------------------------------------------------
int tech_object_manager::evaluate() {
  ...
  if (has_Lua_eval == 2) {
    res = lua_manager::get_instance()->void_exec_lua_method(
        "", "eval", "void tech_object_manager::evaluate()");
  }
...
int tech_object_manager::init_objects() {
  ...
  if (lua_isfunction(lua_manager::get_instance()->get_Lua(), -1)) {
    lua_manager::get_instance()->void_exec_lua_method(
        "", "init", "int tech_object_manager::init_objects()");
  }
