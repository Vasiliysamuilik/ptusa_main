...
class modbus_client {
 protected:
  ...
  static const int write_buff_start =
      13;  //Начало буфера данных для операций записи
  static const int read_buff_start =
      9;  //Начало буфера данных для операций чтения
 public:
  modbus_client(unsigned int id, char* ip, unsigned int port = 502,
  ...
  int async_read_discrete_inputs(unsigned int start_address,
  ...
  int async_write_multiply_registers(unsigned int address,
                                     unsigned int quantity);
  int async_read_write_multiply_registers(unsigned int readaddress,
                                          unsigned int readquantity,
                                          unsigned int wrireaddress,
                                          unsigned int writequantity);
  int async_mask_write_register(unsigned int writeaddress, unsigned int andmask,
                                unsigned int ormask);
