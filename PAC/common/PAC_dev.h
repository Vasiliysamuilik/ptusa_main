...
class par_device {
 ...
 public:
  ...
  saved_params_u_int_4* get_err_par() const { return err_par; }

  /// @brief Установка параметров для хранения настроек ошибок устройства.
  void set_err_par(saved_params_u_int_4* err_par) { this->err_par = err_par; }

  float get_par(int idx) const { return par[0][idx]; }
...
class device : public i_DO_AO_device, public par_device {
 public:
  ...
  enum CONSTANTS { C_MAX_NAME = 20, C_MAX_DESCRIPTION = 100 };
  ...
  enum DEVICE_SUB_TYPE {
    ...
    DST_V_DO1_DI1_FB_OFF,  ///< Клапан с одним каналом управления и одной
                           ///< обратной связью на закрытое состояние.
    DST_V_DO1_DI1_FB_ON,  ///< Клапан с одним каналом управления и одной
                          ///< обратной связью на открытое состояние.
    DST_V_DO1_DI2,  ///< Клапан с одним каналом управления и двумя обратными
                    ///< связями.
    DST_V_DO2_DI2,  ///< Клапан с двумя каналами управления и двумя обратными
                    ///< связями.
    DST_V_MIXPROOF,     ///< Клапан противосмешивающий.
    DST_V_AS_MIXPROOF,  ///< Клапан с двумя каналами управления и двумя
                        ///< обратными связями с AS интерфейсом
                        ///< (противосмешивающий).
    DST_V_BOTTOM_MIXPROOF,  ///< Донный клапан.
    DST_V_AS_DO1_DI2,  ///< Клапан с одним каналом управления и двумя обратными
                       ///< связями с AS интерфейсом.
    V_DO2_DI2_BISTABLE,  ///< Клапан с двумя каналами управления и двумя
                         ///< обратными связями бистабильный.

    V_IOLINK_VTUG_DO1,  ///< IO-Link VTUG8 клапан с одним каналом управления.

    V_IOLINK_VTUG_DO1_FB_OFF,  ///< IO-Link VTUG клапан с одним каналом
                               ///< управления и одной обратной связью
                               ///< (выключенное состояние).
    V_IOLINK_VTUG_DO1_FB_ON,  ///< IO-Link VTUG клапан с одним каналом
                              ///< управления и одной обратной связью
                              ///< (включенное состояние).

    V_IOLINK_MIXPROOF,  ///< Клапан с двумя каналами управления и двумя
                        ///< обратными связями с IO-Link интерфейсом
                        ///< (противосмешивающий).
    V_IOLINK_DO1_DI2,  ///< Клапан с одним каналом управления и двумя обратными
                       ///< связями с IO-Link интерфейсом (отсечной).
    V_IOLINK_VTUG_DO1_DI2,  ///< IO-Link VTUG клапан с одним каналом управления
                            ///< и двумя обратными связями.
    ...
    DST_M_REV,  ///< Мотор с реверсом без управления частотой вращения. Реверс
                ///< включается совместно.
    DST_M_REV_FREQ,  ///< Мотор с реверсом с управлением частотой вращения.
                     ///< Реверс включается совместно.
    DST_M_REV_2,  ///< Мотор с реверсом без управления частотой вращения. Реверс
                  ///< включается отдельно.
    DST_M_REV_FREQ_2,  ///< Мотор с реверсом с управлением частотой вращения.
                       ///< Реверс включается отдельно.
    ...
    DST_LT_TRUNC,  ///Текущий уровень для танка с усеченным цилиндром в
                   ///основании.
    ...
    DST_FS_VIRT,  ///< Виртуальный датчик наличия расхода (без привязки к
                  ///< модулям).
    ...
    // CAM
    DST_CAM_DO1_DI2 =
        1,  ///< C сигналом активации, результатом обработки и готовностью.
    DST_CAM_DO1_DI1,  ///< C сигналом активации и результатом обработки.
    DST_CAM_DO1_DI3,  ///< C сигналом активации, двумя результатами обработки и
                      ///< готовностью.
  ...
  virtual u_int_4 get_serial_n() const { return s_number; }

  /// @brief Установка порядкового номера устройства.
  ///
  /// @return - номер устройства.
  void set_serial_n(u_int_4 s_n) { s_number = s_n; }

  /// @brief Получение типа устройства.
  int get_type() const { return type; }

  /// @brief Получение подтипа устройства.
  int get_sub_type() const { return sub_type; }
  ...
  bool get_manual_mode() const { return is_manual_mode; }

  char* get_article() const { return article; }
  ...
  static int get_max_name_length() { return C_MAX_NAME; }

  /// @brief Получение максимальной длины описания устройства (с учётом
  /// символа завершения строки).
  static int get_max_description_length() { return C_MAX_DESCRIPTION; }
...
class digital_io_device : public device, public io_device {
 public:
...
class analog_io_device : public device, public io_device {
 public:
  analog_io_device(const char* dev_name, device::DEVICE_TYPE type,
                   device::DEVICE_SUB_TYPE sub_type, u_int par_cnt)
      : device(dev_name, type, sub_type, par_cnt), io_device(dev_name) {}
...
#pragma endregion Отключение клапана с задержкой.
  ...
  bool is_wash_seat_active() const { return wash_flag; }
  void set_seat_wash_state(bool wash_flag) { this->wash_flag = wash_flag; }
 ...
 protected:
  ...
  virtual bool get_fb_state() { return true; }
...
#pragma region Отключение клапана с задержкой.
 ...
 private:
  ...
  virtual bool is_switching_off_finished() { return !is_switching_off; };
...
#ifndef DEBUG_NO_IO_MODULES
  int get_off_fb_value() { return get_DI(DI_INDEX); }

  int get_on_fb_value() { return false; }
#endif  // DEBUG_NO_IO_MODULES
};
//-----------------------------------------------------------------------------
/// @brief Клапан с одним дискретным выходом и одним дискретным входом.
...
class valve_DO1_DI1_on : public valve {
 public:
  valve_DO1_DI1_on(const char* dev_name)
      : valve(true, false, dev_name, DT_V, DST_V_DO1_DI1_FB_ON) {}
...
#ifndef DEBUG_NO_IO_MODULES
  int get_on_fb_value() { return get_DI(DI_INDEX); }

  int get_off_fb_value() { return false; }
#endif  // DEBUG_NO_IO_MODULES
};
//-----------------------------------------------------------------------------
...
#ifndef DEBUG_NO_IO_MODULES
  int get_off_fb_value() { return get_DI(DI_INDEX_2); }

  int get_on_fb_value() { return get_DI(DI_INDEX_1); }
#endif  // DEBUG_NO_IO_MODULES
};
//-----------------------------------------------------------------------------
...
#ifndef DEBUG_NO_IO_MODULES
  int get_off_fb_value() { return get_DI(DI_INDEX_1); }

  int get_on_fb_value() { return get_DI(DI_INDEX_2); }
#endif  // DEBUG_NO_IO_MODULES
};
//-----------------------------------------------------------------------------
/// @brief Клапан с двумя каналами управления и двумя обратными связями
 ...
 public:
  ...
  int evaluate() {
    ...
    if (get_DO(DO_INDEX_OPEN) == 1) {
      if (o == 1 || get_delta_millisec(start_switch_time) > t) {
        set_DO(DO_INDEX_OPEN, 0);
      }
    }
    if (get_DO(DO_INDEX_CLOSE) == 1) {
      if (c == 1 || get_delta_millisec(start_switch_time) > t) {
...
#ifndef DEBUG_NO_IO_MODULES
  int get_off_fb_value() { return get_DI(DI_INDEX_CLOSE); }

  int get_on_fb_value() { return get_DI(DI_INDEX_OPEN); }
#endif  // DEBUG_NO_IO_MODULES
...
class valve_AS : public valve {
 public:
  ...
  void print() const { printf("%s [%u]\t", get_name(), AS_number); }
...
class valve_AS_mix_proof : public i_mix_proof, public valve_AS {
 public:
  ...
  void open_upper_seat() { direct_set_state(V_UPPER_SEAT); }

  void open_lower_seat() { direct_set_state(V_LOWER_SEAT); }
};
//-----------------------------------------------------------------------------
...
class valve_AS_DO1_DI2 : public valve_AS {
 public:
  ...
  bool get_fb_state() {
    ...
    if (get_delta_millisec(start_err_time) < get_par(valve::P_ON_TIME, 0)) {
      return true;
    }
...
class valve_iol_terminal : public valve {
 public:
  ...
  void set_st(VALVE_STATE new_state) { state = new_state; }
...
class valve_iol_terminal_mixproof_DO3 : public i_mix_proof,
                                        public valve_iol_terminal {
 public:
  explicit valve_iol_terminal_mixproof_DO3(const char* dev_name, bool is_on_fb,
                                           bool is_off_fb,
                                           device::DEVICE_SUB_TYPE sub_type);

  explicit valve_iol_terminal_mixproof_DO3(
      const char* dev_name,
...
//-----------------------------------------------------------------------------
/// @brief Клапан IO-link VTUG с одним каналом управления и 2-я обратными
/// связями.
...
class valve_iol_terminal_mixproof_DO3_DI2
    : public valve_iol_terminal_mixproof_DO3 {
 public:
...
class AI1 : public analog_io_device {
 public:
  ...
  void direct_set_state(int new_state) { st = new_state; }
#endif  // DEBUG_NO_IO_MODULES
  ...
  virtual int get_params_count() const { return ADDITIONAL_PARAM_COUNT; }
 ...
 protected:
  ...
  virtual float get_max_val() { return 0; }

  /// @brief Получение минимального значения выхода устройства.
  virtual float get_min_val() { return 0; }
...
class temperature_e : public AI1 {
 public:
  temperature_e(const char* dev_name)
      : AI1(dev_name, DT_TE, DST_TE, ADDITIONAL_PARAM_COUNT) {
...
class level : public AI1 {
 public:
  level(const char* dev_name, device::DEVICE_SUB_TYPE sub_type, u_int par_cnt);
  ...
  int get_params_count() const { return start_param_idx + LAST_PARAM_IDX - 1; }
...
class pressure_e : public AI1 {
 public:
  pressure_e(const char* dev_name)
      : AI1(dev_name, DT_PT, DST_NONE, ADDITIONAL_PARAM_COUNT) {
    start_param_idx = AI1::get_params_count();
...
class pressure_e_iolink : public analog_io_device {
 public:
  ...
  static void evaluate_io(const char* name, char* data, ARTICLE n_article,
                          float& v, int& st);
...
class concentration_e_ok : public concentration_e {
 public:
  concentration_e_ok(const char* dev_name)
      : concentration_e(dev_name, DST_QT_OK) {}
...
class analog_input : public AI1 {
 public:
  analog_input(const char* dev_name)
      : AI1(dev_name, DT_AI, DST_NONE, ADDITIONAL_PARAM_COUNT) {
    start_param_idx = AI1::get_params_count();
    set_par_name(P_MIN_V, start_param_idx, "P_MIN_V");
    set_par_name(P_MAX_V, start_param_idx, "P_MAX_V");
  }
...
class wages_eth : public analog_io_device, public i_wages {
 public:
  ...
  void direct_set_tcp_buff(const char* new_value, size_t size, int new_status);
...
class wages : public analog_io_device, public i_wages {
 public:
  ...
  float get_value();
  void direct_set_state(int new_state);
  void direct_set_value(float new_value) { return; }
#endif  // DEBUG_NO_IO_MODULES
...
class AO1 : public analog_io_device {
 public:
  AO1(const char* dev_name, device::DEVICE_TYPE type,
      device::DEVICE_SUB_TYPE sub_type, u_int par_cnt)
...
class analog_output : public AO1 {
 public:
  ...
  float get_min_value() { return get_par(P_MIN_VALUE, 0); }

  float get_max_value() { return get_par(P_MAX_VALUE, 0); }
...
class analog_valve : public AO1 {
 public:
  ...
  float get_min_value() { return C_MIN; }

  float get_max_value() { return C_MAX; }
...
class valve_DO2 : public valve {
 public:
  ...
  /// @brief Получение состояния обратной связи.
  bool get_fb_state() override { return true; }
...
class motor : public i_motor, public io_device {
 public:
  ...
  virtual void print() const { device::print(); }
...
class motor_altivar : public i_motor, public io_device {
 public:
  ...
  float get_rpm() const { return (float)rpm; }
#endif  // DEBUG_NO_IO_MODULES

  altivar_node* get_atv() const { return atv; }
...
class flow_s : public DI1 {
 public:
  flow_s(const char* dev_name) : DI1(dev_name, DT_FS, DST_NONE, 0) {}
};
//-----------------------------------------------------------------------------
/// @brief Датчик положения.
class state_s : public DI1 {
 public:
  explicit state_s(const char* dev_name) : DI1(dev_name, DT_GS, DST_GS, 0) {}
};
//-----------------------------------------------------------------------------
/// @brief Датчик положения инверсный.
class state_s_inverse : public DI1 {
 public:
  explicit state_s_inverse(const char* dev_name)
      : DI1(dev_name, DT_GS, DST_GS_INVERSE, 0, 1) {}

  bool is_active() override { return get_state() == 0 ? true : false; }
};
//-----------------------------------------------------------------------------
/// @brief Датчик разности давления.
class diff_pressure : public DI1 {
 public:
  diff_pressure(const char* dev_name)
      : DI1(dev_name, DT_PDS, DST_PDS_VIRT, 0) {}
};
//-----------------------------------------------------------------------------
/// @brief Сигнальный датчик температуры.
class temperature_signal : public DI1 {
 public:
  temperature_signal(const char* dev_name)
      : DI1(dev_name, DT_TS, DST_TS_VIRT, 0) {}
};
//-----------------------------------------------------------------------------
...
class counter : public base_counter {
 public:
  counter(const char* dev_name, DEVICE_SUB_TYPE sub_type = device::DST_FQT,
...
class signal_column_discrete : public signal_column {
 public:
  signal_column_discrete(const char* dev_name, int red_lamp_channel = 1,
                         int yellow_lamp_channel = 2,
...
class camera : public i_camera, public device, public io_device {
 public:
  camera(const char* dev_name, DEVICE_SUB_TYPE sub_type, int params_count = 0,
         bool is_ready = true);
...
class power_unit : public analog_io_device {
 public:
  power_unit(const char* dev_name, device::DEVICE_SUB_TYPE sub_type =
                                       device::DEVICE_SUB_TYPE::DST_G_IOL_4);
...
#ifdef PTUSA_TEST
  void set_cmd_time(unsigned long t) { cmd_time = t; }
...
class device_manager : public i_Lua_save_device {
 public:
  ...
  const char* get_name_in_Lua() const { return "Device manager"; }

  /// @brief Получение фиктивного устройства.
  dev_stub* get_stub() { return &stub; }
...
#pragma option -w.inl
 ...
 protected:
  ...
  int get_device_n(device::DEVICE_TYPE dev_type, const char* dev_name);
 ...
 public:
  ...
  //
  // Вызывается из Lua.
  io_device* add_io_device(int dev_type, int dev_sub_type, const char* dev_name,
                           const char* descr, const char* article);
...
class timer_manager {
 public:
  ...
  int get_count() const { return timers_cnt; }
