...
#define P_CKANAL_S \
  3  //максимальная концентрация раствора щелочи для канализации
...
#define P_CKANAL_K \
  6  //максимальная концентрация раствора кислоты для канализации
#define P_NAV_OVERREGULATE \
  7  //Перерегулирование концентрации при наведении растворов
...
#define P_MAX_BULK_FOR_CAUSTIC \
  31  //максимальный аналоговый уровень при наполнении танка щелочи при
      //наведении
#define P_MAX_BULK_FOR_ACID \
  32  //максимальный аналоговый уровень при наполнении танка кислоты при
      //наведении
...
#define P_MIN_BULK_FOR_WATER \
  39  //Минимальный аналоговый уровень в танке воды, при котором считать, что
      //его нет
...
enum workParameters {
  ...
  P_DELTA_TR,  //разница температур на подаче и возврате (если меньше заданной,
               //то начинается отсчет времени циркуляции)
  ...
  P_V_RAB_ML,  //количество литров выше среднего уровня при котором
               //заканчивается заполнение бачка циркуляции
  PV_RET_DEL,  //через сколько литров после начала подачи V2 включать возвратный
               //насос
  ...
  PTM_SANITIZER_INJECT,  //время внесения дезинфицирующего средства при
                         //дезинфекции
  ...
  P_T_SANITIZER_RINSING,  //Температура ополаскивания после мойки
                          //дезинфецирующим средством
  P_V_SANITIZER_RINSING,  //Объем ополаскивания после мойки дезинфицирующим
                          //средством
  P_TM_MAX_TIME_OPORBACHOK,  //Максимальное время от пропадания нижнего уровня в
                             //бачке до завершении операции опорожнения бачка

  P_TM_RET_IS_EMPTY,  //время отсутствия протока на возврате для окончания
                      //опорожнения объекта
  P_V_LL_BOT,  //количество литров от пропадания нижнего уровня в бачке до
               //завершения операции "опорожнение бачка"
  ...
  PIDF_Uk,       //Выход ПИД.
  //-PID2-!>
  P_TM_MAX_TIME_OPORCIP,  //Максимальное время операции "Опорожнение объекта
                          //CIP"
  ...
  P_SIGNAL_SANITIZER_PUMP,  //Сигнал out управления дозатором дезинфицирующего
                            //средства
  P_RESUME_CIP_ON_SIGNAL,  //Автоматическое возобновление мойки при пропадании
                           //ошибки объекта CIP
  P_SIGNAL_PUMP_CONTROL,  //Сигнал in analog "Управление производительностью
                          //подающего насоса"
  ...
  P_SIGNAL_PUMP_CONTROL_FEEDBACK,  //Сигнал in analog "Контролируемый уровень
                                   //для насоса подачи"
  P_SIGNAL_RET_PUMP_SENSOR,  //Сигнал датчика уровня для работы возвратного
                             //насоса
  P_RET_PUMP_SENSOR_DELAY,  //Задержка выключения возвратного насоса при
                            //пропадании сигнала датчика уровня
...
enum SELFCLEAN_PARAMS {
  ...
  SCP_T_SCH_CIRC_PREDV,  //Время предварительной циркуляции щелочи в танке
                         //щелочи
...
class cipline_tech_object : public tech_object {
 protected:
  ...
  bool is_ready_to_end{};  //Операция должна закончиться но ожидает сигнала
                           //готовности от объекта

  unsigned int tech_type;  //подтип объекта
  int ret_circ_flag;  //флаг управления возвратным насосом при циркуляции
  unsigned long
      ret_circ_delay;  //задержка обратного включения возвратного насоса

  unsigned long
      steam_valve_delay;  //таймер задержки включения отсечного клапана пара
                          //(чтобы не включался/выключался на граничных
                          //значениях температуры)
  unsigned long
      bachok_lvl_err_delay;  //задержка для появления ошибки уровня бачка
  ...
  unsigned long sort_delay;  //таймер задержки при сортировке растворов (чтобы
                             //не переключалось хаотически)
  ...
  int forcesortrr;  //флаг принудительной сортировки растворов без учета
                    //таймаута переключений
 ...
 public:
  cipline_tech_object(const char* name, u_int number, u_int type,
                      const char* name_Lua, u_int states_count,
                      u_int timers_count, u_int par_float_count,
                      u_int runtime_par_float_count, u_int par_uint_count,
  ...
  char circ_water_no_pump_stop;  //не останавливать насос при поялении верхнего
                                 //уровня в бачке
  char circ_medium_no_pump_stop;  //не останавливать насос при поялении верхнего
                                  //уровня в бачке на щелочи и кислоте
  ...
  unsigned long
      default_programlist;  //список доступных программ мойки по-умолчанию
  int disable_tank_heating;  //отключение подогрева при начале подачи растворов
                             //в танк(для МСА со старыми регулирующими
                             //клапанами)
  int clean_water_rinsing_return;  //Куда возвращать на операции окончательного
                                   //ополаскивания
  ...
  int SCInitPumping(int what, int from, int where, int whatdrainage, int step,
                    int f);
  ...
  device* dev_os_can_continue;  //Сигнал "можно переходить на другой шаг" на
                                //операциях циркуляции и доп. ополаскиваниии
  ...
  device*
      dev_upr_pump_stopped;  //Сигнал "подающий насос остановлен и нет потока"
  ...
  int init_object_devices();  //Функция для инициализации устройств объекта
                              //мойки
  ...
  ///Базовые функции. При необходимости могут вызываться из Lua. Могут быть
  ///переопределены.
  int _DoStep(int step_to_do);  //Выполнение шага, заданного параметром.
  int _GoToStep(int cur, int param);  //Переход к следующему шагу.
  int _InitStep(int step_to_init,
                int not_first_call);  // cip_InitStep(steptoinit, param)
  int _LoadProgram();
  void _StopDev();
