...
class io_device {
 ...
 private:
  ...
  struct IO_channels {
    ...
    int* module_offsets;  ///< Массив смещений начала адресного пространства
                          ///< модуля IO для канала
    int* logical_ports;  ///< Массив логических номеров канала в пределах модуля
                         ///< IO

    int_2** int_read_values;  ///< Массив значений для чтения.
    int_2** int_module_read_values;  ///< Массив значений для чтения адресного
                                     ///< пространства модуля.
    ...
    void init_channel(u_int ch_index, int node, int offset, int module_offset,
                      int logical_port);
...
class io_manager {
 public:
  ...
  struct io_node {
    ...
    enum W_CONST {
      C_MAX_WAIT_TIME =
          6000,  ///< Время до установки ошибки связи с модулем, мсек.
      ...
      C_CNT_TIMEOUT_US =
          100000,  ///< Время ожидания подключения от модуля, мксек.
      C_RCV_TIMEOUT_US = 250000,  ///< Время ожидания ответа от модуля, мксек.
      C_INITIAL_RECONNECT_DELAY =
          500,  ///< Изначальная задержка переподключения к узлу при ошибке
                ///< связи, мсек.
    };
 ...
 public:
  ...
  void add_node(u_int index, int ntype, int address, const char* IP_address,
                const char* name, int DO_cnt, int DI_cnt, int AO_cnt,
                int AO_size, int AI_cnt, int AI_size);

  /// @brief Инициализация параметров канала аналогового вывода.
  ///
  /// Вызывается из Lua.
  void init_node_AO(u_int node_index, u_int AO_index, u_int type, u_int offset);

  /// @brief Инициализация параметров канала аналогового ввода.
  ///
  /// Вызывается из Lua.
  void init_node_AI(u_int node_index, u_int AI_index, u_int type, u_int offset);
