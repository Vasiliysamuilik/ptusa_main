...
{
  if (index < DO_channels.count && DO_channels.char_write_values &&
  ...
  if (G_DEBUG) {
    ...
    printf("index = %d, count = %u, char_write_values = %p", index,
           DO_channels.count, DO_channels.char_write_values);
...
int io_device::set_DO(u_int index, char value) {
  ...
  if (G_DEBUG) {
    print();
    printf("io_device->set_DO(...) - error! %d, %d, %p", index,
           DO_channels.count, DO_channels.char_write_values);
    if (DO_channels.char_write_values) {
      printf(", char_write_values[ index ]=%p",
             DO_channels.char_write_values[index]);
...
int io_device::get_DI(u_int index) {
  if (index < DI_channels.count && DI_channels.char_read_values &&
...
float io_device::get_AO(u_int index, float min_value, float max_value) {
  if (index < AO_channels.count && AO_channels.int_write_values &&
    ...
    switch (module_type) {
      ...
      //    0.005           4.0039          0000 0000 0000 1xxx     00 08 8 0 4
      //    0000 0000 0000 0111     00 07         7 0               4 0000 0000
      //    0000 0000     00 00         0
      //
      case 554:
      case 555:
...
float io_device::get_AI(u_int index, float min_value, float max_value) {
  if (index < AI_channels.count && AI_channels.int_read_values &&
    ...
    switch (module_type) {
      ...
      //   -------------------------------------------------------------------------
      //                >400
      //   850          390.481     1384,998    0010 0001 0011 0100     2134
      //   8500 100          138.506     1099,299    0000 0011 1110 1000 03E8
      //   1000 25.5         109.929     1000,391    0000 0000 1111 1111 00FF
      //   255 0.1          100.039     1000        0000 0000 0000 0001     0001
      //   1 0            100         999,619     0000 0000 0000 0000     0000 0
      //  -0.1          99.970      901,929     1111 1111 1111 1111     FFFF -1
      //  -25.5         90.389      184,936     1111 1111 0000 0001     FF01
      //  -255 -200          18.192                  1111 1000 0011 0000 F830
      //  -2000
      //                <18                     1000 0000 0000 0000     8000
      //                -32767
      //
      case 461:
      case 450:
        ...
        //    Dec.
        //    -----------------------------------------------------------------------
        //   >20.5           >20.5            0111 1111 1111 1111     7F FF
        //   32767
        //    20              20              0111 1111 1111 1111     7F FF
        //    32767 10              12              0100 0000 0000 0xxx     40
        //    00     16384 5               8               0010 0000 0000 0xxx
        //    20 00      8192 2.5             6               0001 0000 0000
        //    0xxx     10 00      4096 0.156           4.125           0000 0001
        //    0000 0xxx     01 00       256 0.01            4.0078          0000
        //    0000 0001 0xxx     00 10        16 0.005           4.0039 0000
        //    0000 0000 1xxx     00 08         8 0               4 0000 0000
        //    0000 0111     00 07         7 0               4               0000
        //    0000 0000 0000     00 00         0
        //
      case 466:
      case 496:
        if (3 == val) {
...
io_device::IOLINKSTATE io_device::get_AI_IOLINK_state(u_int index) {
  if (vendor == PHOENIX) {
    if (index < AI_channels.count &&
        AI_channels.int_module_read_values[index]) {
      u_int module_states =
          *((u_int*)(AI_channels.int_module_read_values[index]));
...
io_device::IOLINKSTATE io_device::get_AO_IOLINK_state(u_int index) {
  if (vendor == PHOENIX) {
    if (index < AO_channels.count &&
        AO_channels.int_module_read_values[index]) {
      u_int module_states =
          *((u_int*)(AO_channels.int_module_read_values[index]));
...
//-----------------------------------------------------------------------------
io_device::io_device(const char* name)
    : name(name),
      vendor(PHOENIX),
...
io_device::~io_device() {}
//-----------------------------------------------------------------------------
void io_device::init(int DO_count, int DI_count, int AO_count, int AI_count) {
  if (DO_count > 0) {
...
//-----------------------------------------------------------------------------
io_device::VENDOR io_device::get_io_vendor() const { return vendor; }
//-----------------------------------------------------------------------------
void io_device::set_io_vendor(VENDOR vendor) { this->vendor = vendor; }
//-----------------------------------------------------------------------------
void io_device::init_channel(int type, int ch_inex, int node, int offset,
  ...
  switch (type) {
    case IO_channels::CT_DI:
      DI_channels.init_channel(ch_inex, node, offset, module_offset,
                               logical_port);
      break;

    case IO_channels::CT_DO:
      DO_channels.init_channel(ch_inex, node, offset, module_offset,
                               logical_port);
      break;

    case IO_channels::CT_AI:
      AI_channels.init_channel(ch_inex, node, offset, module_offset,
                               logical_port);
      break;

    case IO_channels::CT_AO:
      AO_channels.init_channel(ch_inex, node, offset, module_offset,
                               logical_port);
      break;
  }
}
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
io_device::IO_channels::IO_channels(CHANNEL_TYPE type)
    : count(0),
      tables(0),
      offsets(0),
      module_offsets(0),
...
void io_device::IO_channels::init_channel(u_int ch_index, int node, int offset,
  ...
  if (ch_index < count) {
    ...
    switch (type) {
      case CT_DI:
        char_read_values[ch_index] =
            io_manager::get_instance()->get_DI_read_data(tables[ch_index],
                                                         offsets[ch_index]);
        break;

      case CT_DO:
        char_read_values[ch_index] =
            io_manager::get_instance()->get_DO_read_data(tables[ch_index],
                                                         offsets[ch_index]);
        char_write_values[ch_index] =
            io_manager::get_instance()->get_DO_write_data(tables[ch_index],
                                                          offsets[ch_index]);
        break;

      case CT_AI:
        int_read_values[ch_index] =
            io_manager::get_instance()->get_AI_read_data(tables[ch_index],
                                                         offsets[ch_index]);
        if (module_offsets[ch_index] >= 0) {
          int_module_read_values[ch_index] =
              io_manager::get_instance()->get_AI_read_data(
                  tables[ch_index], module_offsets[ch_index]);
        } else {
      ...
      case CT_AO:
        int_read_values[ch_index] =
            io_manager::get_instance()->get_AO_read_data(tables[ch_index],
                                                         offsets[ch_index]);
        int_write_values[ch_index] =
            io_manager::get_instance()->get_AO_write_data(tables[ch_index],
                                                          offsets[ch_index]);
        if (module_offsets[ch_index] >= 0) {
          int_module_read_values[ch_index] =
              io_manager::get_instance()->get_AI_read_data(
    ...
    if (G_DEBUG) {
      printf(
          "Error io_device::IO_channels::init_channel - index out of bound!\n");
    }
  }
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
io_manager::io_node* io_manager::get_node(int node_n) { return nodes[node_n]; }
u_int io_manager::get_nodes_count() { return nodes_count; }
//-----------------------------------------------------------------------------
void io_manager::add_node(u_int index, int ntype, int address,
                          const char* IP_address, const char* name, int DO_cnt,
                          int DI_cnt, int AO_cnt, int AO_size, int AI_cnt,
                          int AI_size) {
  if (index < nodes_count) {
    nodes[index] = new io_node(ntype, address, IP_address, name, DO_cnt, DI_cnt,
                               AO_cnt, AO_size, AI_cnt, AI_size);
  }
}
//-----------------------------------------------------------------------------
void io_manager::init_node_AO(u_int node_index, u_int AO_index, u_int type,
                              u_int offset) {
...
//-----------------------------------------------------------------------------
void io_manager::init_node_AI(u_int node_index, u_int AI_index, u_int type,
                              u_int offset) {
...
void io_manager::disconnect(io_node* node) { return; }
//-----------------------------------------------------------------------------
...
io_manager::io_node::io_node(int type, int number, const char* str_ip_address,
                             const char* name, int DO_cnt, int DI_cnt,
  ...
  if (ip_address[0] == 0 && type >= WAGO_750_XXX_ETHERNET) {
    is_active = false;
    sprintf(G_LOG->msg, "Узел \"%s\" отключен, так как не задан его IP адрес.",
            name);
  ...
  if (type == EMPTY) {
    is_active = false;
    sprintf(G_LOG->msg, "Узел \"%s\" отключен, так как не задан его тип.",
            name);
    G_LOG->write_log(i_log::P_NOTICE);
  }
...
void io_manager::io_node::print() {
  printf(
      "\"%s\" - type %d, number %d, IP \"%s\", "
      "DI %d [%d], DO %d [%d], AI %d [%d], AO %d [%d].\n",
      name, type, number, ip_address, DI_cnt, DI_cnt / CHAR_BIT, DO_cnt,
      DO_cnt / CHAR_BIT, AI_cnt, AI_size, AO_cnt, AO_size);
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
io_manager* G_IO_MANAGER() { return io_manager::get_instance(); }
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#ifdef WIN_OS
#pragma warning(pop)
