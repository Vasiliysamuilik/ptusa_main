...
std::unordered_set<int> cipline_tech_object::steps_v2_supply = {7, 35, 55, 64,
                                                                85};

std::unordered_set<int> cipline_tech_object::steps_additional_rinse = {8, 37,
                                                                       57, 86};

std::unordered_set<int> cipline_tech_object::steps_circulation = {28, 48, 66,
                                                                  76, 77};

std::unordered_set<int> cipline_tech_object::steps_caustic = {
    22, 23, 24, 26, 28, 29, 31, 33, 34, 35};

std::unordered_set<int> cipline_tech_object::steps_acid = {42, 43, 44, 46, 48,
                                                           49, 53, 54, 55};
...
cipline_tech_object::cipline_tech_object(const char* name, u_int number,
                                         u_int type, const char* name_Lua,
                                         u_int states_count, u_int timers_count,
                                         u_int par_float_count,
                                         u_int runtime_par_float_count,
                                         u_int par_uint_count,
                                         u_int runtime_par_uint_count)
    : tech_object(name, number, type, name_Lua, states_count, timers_count,
                  par_float_count, runtime_par_float_count, par_uint_count,
  ...
  loadedRecName =
      new char[TRecipeManager::recipeNameLength * UNICODE_MULTIPLIER];
  ...
  clean_water_rinsing_return =
      TANK_W;  //по-умолчанию возвращаем в танк со вторичной водой.
  disable_final_rinsing = false;

  if (tech_type == TECH_TYPE_SELF_CLEAN ||
      tech_type == TECH_TYPE_CAR_WASH_SELF_CLEAN) {
    scenabled = 1;
    scline = 0;
    resetProgramList();
    VSDREN = V(1);
    ...
    if (nullptr == scparams) {
      scparams =
          new saved_params<float, true>(SELFCLEAN_PAR_COUNT, "PAR_SELFCLEAN");
    }
  } else {
    scparams = parpar;
  }
...
int cipline_tech_object::save_device(char* buff) {
  ...
  if (nmr == 1) {
    ...
    answer_size +=
        sprintf(buff + answer_size, "\tCAUSTICNAME='%s',\n", causticName);
    ...
    for (i = 1; i <= acidRecipes->GetParamsCount(); i++) {
      answer_size +=
          sprintf(buff + answer_size, "%.2f, ", acidRecipes->getValue(i - 1));
    }
    answer_size += sprintf(buff + answer_size, "\n\t}\n");
  }
  ...
  if (strcmp(prop, "RT_PAR_F") == 0) {
    ...
    if (idx >= PIDP_Z && idx <= PIDF_Uk) {
      if (state && loadedRecipe >= 0) {
        lineRecipes->setRecipeValue(
            loadedRecipe, idx - PIDP_Z + TRecipeManager::RV_PIDP_Z, float(val));
      }
    }
    return 0;
  }
  ...
  if (0 == strcmp(prop, "CUR_REC")) {
    ...
    if (slen < (unsigned int)TRecipeManager::recipeNameLength) {
      ...
      strncpy_s(lineRecipes->currentRecipeName,
                TRecipeManager::recipeNameLength * UNICODE_MULTIPLIER, val,
                _TRUNCATE);
#else
  ...
  if (0 == strcmp(prop, "NCAR")) {
    ...
    if (slen < CAR_NAME_MAX_LENGTH) {
      switch (idx) {
        ...
        case 1:
#ifdef WIN_OS
          strncpy_s(ncar1, CAR_NAME_MAX_LENGTH * UNICODE_MULTIPLIER, val,
                    _TRUNCATE);
#else
        ...
        case 2:
#ifdef WIN_OS
          strncpy_s(ncar2, CAR_NAME_MAX_LENGTH * UNICODE_MULTIPLIER, val,
                    _TRUNCATE);
#else
        ...
        case 3:
#ifdef WIN_OS
          strncpy_s(ncar3, CAR_NAME_MAX_LENGTH * UNICODE_MULTIPLIER, val,
                    _TRUNCATE);
#else
        ...
        case 4:
#ifdef WIN_OS
          strncpy_s(ncar4, CAR_NAME_MAX_LENGTH * UNICODE_MULTIPLIER, val,
                    _TRUNCATE);
#else
...
void cipline_tech_object::initline() {
  ...
  if (((device*)V(devname))->get_type() == device::DT_NONE ||
      ((device*)V(devname2))->get_type() == device::DT_NONE) {
    is_old_definition = 0;
  }
  ...
  PIDF = new MSAPID(&rt_par_float, PIDF_Z, P_ZAD_FLOW, PUMPFREQ, nullptr, cnt,
                    P_PIDF_MAX_OUT);
  PIDP = new MSAPID(&rt_par_float, PIDP_Z, P_ZAD_PODOGR, ao, TP, nullptr,
                    P_PIDP_MAX_OUT);
  ...
  check_Lua_function("cip_ResetLinesDevicesBeforeReset",
                     is_ResetLinesDevicesBeforeReset_func);
  ...
  if (nmr == 1) {
    ...
    if (causticLoadedRecipe >= 0 &&
        causticLoadedRecipe < TMediumRecipeManager::recipePerLine) {
    ...
    if (acidLoadedRecipe >= 0 &&
        acidLoadedRecipe < TMediumRecipeManager::recipePerLine) {
...
void cipline_tech_object::resetProgramName() {
  snprintf(currentProgramName, PROGRAM_MAX_LEN * UNICODE_MULTIPLIER,
           "Не выбран");
}
...
void cipline_tech_object::resetProgramList(
    unsigned long programmask /*= 0xB00*/) {
  prgListLen = 0;
  ...
  if (scenabled && scline == 0 &&
      (SPROG_SELF_CLEAN & programmask) == SPROG_SELF_CLEAN) {
    addProgramToList("Очистка танков", SPROG_SELF_CLEAN);
  }
}

void cipline_tech_object::addProgramToList(const char* programName,
                                           int programNmr) {
...
void cipline_tech_object::formProgramList(unsigned long programmask) {
  ...
  if (((programmask >> 16) & 1) &&
      (static_cast<int>(rt_par_float[P_PROGRAM]) & (1 << PRG_D))) {
...
int cipline_tech_object::isTank() {
  if ((((int)rt_par_float[P_OBJ_TYPE]) & 0xFF) == 1 ||
      (((int)rt_par_float[P_OBJ_TYPE]) & 0xFF) == 3) {
    return 1;
  } else {
    return 0;
  }
...
int cipline_tech_object::isLine() {
  if ((((int)rt_par_float[P_OBJ_TYPE]) & 0xFF) == 2 ||
      (((int)rt_par_float[P_OBJ_TYPE]) & 0xFF) == 4) {
    return 1;
  } else {
    return 0;
  }
...
int cipline_tech_object::getValvesConflict() const {
  int i, j, k;
  for (i = TRecipeManager::RV_FIRSTVALVEON;
       i <= TRecipeManager::RV_LASTVALVEOFF; i++) {
    if (lineRecipes->getRecipeValue(loadedRecipe, i) != 0) {
      for (j = 0; j < MdlsCNT; j++) {
        if (Mdls[j]->nmr != nmr && Mdls[j]->state != 0 &&
            Mdls[j]->loadedRecipe >= 0) {
          if (i <= TRecipeManager::RV_LASTVALVEON) {
            for (k = TRecipeManager::RV_FIRSTVALVEON;
                 k <= TRecipeManager::RV_LASTVALVEOFF; k++) {
              if (lineRecipes->getRecipeValue(loadedRecipe, i) ==
                      Mdls[j]->lineRecipes->getRecipeValue(
                          Mdls[j]->loadedRecipe, k) &&
                  lineRecipes->getRecipeValue(loadedRecipe, i) < 1000) {
                if (G_DEBUG) {
                  printf(
                      "Opened valve %d on line %d conflicts with valve on line "
                      "%d",
                      (int)lineRecipes->getRecipeValue(loadedRecipe, i), nmr,
                      Mdls[j]->nmr);
                }
                return 1;
              }
            ...
            for (k = TRecipeManager::RV_FIRSTVALVEON;
                 k <= TRecipeManager::RV_LASTVALVEON; k++) {
              if (lineRecipes->getRecipeValue(loadedRecipe, i) ==
                      Mdls[j]->lineRecipes->getRecipeValue(
                          Mdls[j]->loadedRecipe, k) &&
                  lineRecipes->getRecipeValue(loadedRecipe, i) < 1000) {
                if (G_DEBUG) {
                  printf(
                      "Closed valve %d on line %d conflicts with opened valve "
                      "on line %d",
                      (int)lineRecipes->getRecipeValue(loadedRecipe, i), nmr,
                      Mdls[j]->nmr);
                }
...
int cipline_tech_object::EvalRecipes() {
  if (1 == nmr && use_internal_medium_recipes) {
    ...
    if ((int)(parpar[0][P_CAUSTIC_SELECTED]) != causticLoadedRecipe) {
      ...
      if (newcausticrecipe >= 0 &&
          newcausticrecipe < causticRecipes->recipePerLine) {
  ...
  if (rt_par_float[P_SELECT_REC] > 0) {
    lineRecipes->LoadRecipeToParams((int)rt_par_float[P_SELECT_REC] - 1, 2, 16,
                                    103, &rt_par_float);
  ...
  switch (ncmd) {
    case MCMD_NEXT:
      if (state != 0 && curstep != 555) {
        rt_par_float[STP_STEPS_OVER] =
            rt_par_float[STP_STEPS_OVER] + 1;  //количество пропущенных операций
    ...
    case MCMD_EVALUATE:
      ...
      if (state == 0 &&
        ...
        if (scenabled) {
          if (scline > 0 &&
              scline != nmr)  //если идет самоочистка танков, то нельзя включить
                              //мойку на других линиях.
          {
            return 0;
          }
          if (0 == scline &&
              SPROG_SELF_CLEAN ==
                  rt_par_float[P_PROGRAM])  //нельзя включить самоочистку, если
                                            //на других линиях идет мойка
          {
          ...
          if (SPROG_SELF_CLEAN == rt_par_float[P_PROGRAM]) {
            scline = nmr;
            for (int tmpline = 0; tmpline < MdlsCNT; tmpline++) {
              if (Mdls[tmpline]->nmr != nmr) Mdls[tmpline]->resetProgramList();
            }
          }
        }

        if (TECH_TYPE_CAR_WASH == tech_type ||
            TECH_TYPE_CAR_WASH_SELF_CLEAN == tech_type) {
          ...
          if (rt_par_float[P_PROGRAM] == SPROG_CAUSTIC_PREPARATION ||
              rt_par_float[P_PROGRAM] == SPROG_ACID_PREPARATION ||
              rt_par_float[P_PROGRAM] == SPROG_SELF_CLEAN) {
            switch1 = 0;
            switch2 = 0;
        ...
        if (valvesAreInConflict) {
          ...
          if (init_object_devices()) {
            ...
            if (no_acid_wash_max > 0 &&
                objectstats->objcausticwashes > no_acid_wash_max &&
                !is_acid_program && is_caustic_program) {
              set_err_msg("Необходима мойка кислотой", 0, 0,
                          ERR_MSG_TYPES::ERR_ALARM);
    ...
    case MCMD_RELOAD_WASH_STATS:
      ...
      for (i = 0; i < MdlsCNT; i++) {
        if (Mdls[i]->state != 0 &&
            Mdls[i]->rt_par_float[P_PROGRAM] != SPROG_CAUSTIC_PREPARATION &&
            Mdls[i]->rt_par_float[P_PROGRAM] != SPROG_ACID_PREPARATION &&
            Mdls[i]->rt_par_float[P_PROGRAM] != SPROG_SELF_CLEAN) {
          if ((tech_type != TECH_TYPE_CAR_WASH) &&
              (tech_type != TECH_TYPE_CAR_WASH_SELF_CLEAN)) {
            Mdls[i]->objectstats = statsbase->stats_if_exists(
                Mdls[i]->loadedRecName, Mdls[i]->emptystats);
            Mdls[i]->objectstats->changed = 0;
          } else {
            Mdls[i]->objectstats =
                statsbase->stats_if_exists(Mdls[i]->ncar1, Mdls[i]->emptystats);
            Mdls[i]->objectstats->changed = 0;
          }
        }
      }
  ...
  switch (cur) {
    ...
    case 61:
      if ((int)(rt_par_float[P_PROGRAM]) & KS_MASK) {
        rt_par_float[STP_USED_HOTWATER] =
            rt_par_float[STP_USED_HOTWATER] + rt_par_float[PV1] +
            rt_par_float[PV2];  //Если в программе присутствует мойка щелочью
                                //или кислотой, то контур на дезинфекции уже
                                //заполнен чистой водой.
        return 66;
      }
      return 62;
    case 62:
...
int cipline_tech_object::_InitStep(int step_to_init, int not_first_call) {
  ...
  switch (step_to_init) {
    case 0:  // INIT fill circ tank
      ...
      return 0;
    case 5:
      return InitToObject(tank_w_src, KANAL, step_to_init, not_first_call);
    case 6:
      return InitOporCIP(KANAL, step_to_init, not_first_call);
    case 7:
      return InitFromObject(tank_w_src, KANAL, step_to_init, not_first_call);
    case 8:
      return InitToObject(tank_w_src, KANAL, step_to_init, not_first_call);
    case 9:
      return InitOporCIP(KANAL, step_to_init, not_first_call);
    case 10:
      return InitFilCirc(WITH_RETURN, step_to_init, not_first_call);
    case 11:
      return InitCirc(WATER, step_to_init, not_first_call);
    case 12:
      return InitOporCirc(KANAL, step_to_init, not_first_call);
    case 13:
      return InitFilCirc(WITH_RETURN, step_to_init, not_first_call);
    case 14:
      return InitToObject(tank_w_src, KANAL, step_to_init, not_first_call);
    ...
    case 16:
      return InitOporCirc(KANAL, step_to_init, not_first_call);
    case 22:
      return InitToObject(TANK_S, KANAL, step_to_init, not_first_call);
    case 23:
      return InitOporCIP(KANAL, step_to_init, not_first_call);
    case 24:
      return InitFromObject(TANK_S, KANAL, step_to_init, not_first_call);
    case 26:
      return InitFilCirc(WITH_RETURN, step_to_init, not_first_call);
    case 28:
      return InitCirc(SHCH, step_to_init, not_first_call);
    case 29:
      return InitOporCirc(TANK_S, step_to_init, not_first_call);

    case 31:
      return InitFilCirc(WITH_WATER, step_to_init, not_first_call);
    case 33:
      return InitToObject(pr_media, TANK_S, step_to_init, not_first_call);
    case 34:
      return InitOporCIP(TANK_S, step_to_init, not_first_call);
    case 35:
      return InitFromObject(pr_media, TANK_S, step_to_init, not_first_call);
    case 37:
      return InitToObject(pr_media, tank_w_dest, step_to_init, not_first_call);
    case 39:
      return InitOporCirc(tank_w_dest, step_to_init, not_first_call);
    case 40:
      return InitOporCIP(tank_w_dest, step_to_init, not_first_call);

    case 42:
      return InitToObject(TANK_K, KANAL, step_to_init, not_first_call);
    case 43:
      return InitOporCIP(KANAL, step_to_init, not_first_call);
    case 44:
      return InitFromObject(TANK_K, KANAL, step_to_init, not_first_call);
    case 46:
      return InitFilCirc(WITH_RETURN, step_to_init, not_first_call);
    case 48:
      return InitCirc(KISL, step_to_init, not_first_call);
    case 49:
      return InitOporCirc(TANK_K, step_to_init, not_first_call);
    case 53:
      return InitToObject(WATER, TANK_K, step_to_init, not_first_call);
    case 54:
      return InitOporCIP(TANK_K, step_to_init, not_first_call);
    case 55:
      return InitFromObject(WATER, TANK_K, step_to_init, not_first_call);
    case 57:
      return InitToObject(WATER, tank_w_dest, step_to_init, not_first_call);
    case 59:
      return InitOporCirc(tank_w_dest, step_to_init, not_first_call);
    case 60:
      return InitOporCIP(tank_w_dest, step_to_init, not_first_call);
    ...
    case 63:
      return InitOporCIP(tank_w_dest, step_to_init, not_first_call);
    case 64:
      return InitFromObject(WATER, tank_w_dest, step_to_init, not_first_call);
    case 65:
      return InitFilCirc(WITH_WATER, step_to_init, not_first_call);
    case 66:
      return InitCirc(HOT_WATER, step_to_init, not_first_call);
    case 67:
      return InitOporCirc(tank_w_dest, step_to_init, not_first_call);

    case 71:
      return InitFilCirc(SANITIZER, step_to_init, not_first_call);
    case 72:
      return InitToObject(SANITIZER, tank_w_dest, step_to_init, not_first_call);
    case 73:
      return InitOporCIP(tank_w_dest, step_to_init, not_first_call);
    case 74:
      return InitFromObject(SANITIZER, tank_w_dest, step_to_init,
                            not_first_call);
    case 75:
      return InitFilCirc(SANITIZER, step_to_init, not_first_call);
    case 76:
      return InitDoseRR(SANITIZER, step_to_init, not_first_call);
    case 77:
      return InitCirc(SANITIZER, step_to_init, not_first_call);
    case 78:
      return InitOporCirc(KANAL, step_to_init, not_first_call);
    case 79:
      return InitOporCIP(KANAL, step_to_init, not_first_call);
    ...
    case 81:
      return InitFilCirc(WITH_WATER, step_to_init, not_first_call);
    case 83:
      return InitToObject(WATER, cleanrinsingto, step_to_init, not_first_call);
    case 84:
      return InitOporCIP(cleanrinsingto, step_to_init, not_first_call);
    case 85:
      return InitFromObject(WATER, cleanrinsingto, step_to_init,
                            not_first_call);
    case 86:
      return InitToObject(WATER, cleanrinsingto, step_to_init, not_first_call);
    case 91:
      return InitOporCIP(cleanrinsingto, step_to_init, not_first_call);
    case 105:
      return InitFilRR(TANK_S);
    case 106:
      return InitCircRR(TANK_S);
    case 108:
      return InitCheckConc(TANK_S);
    case 109:
      return InitAddRR(TANK_S, step_to_init, not_first_call);
    case 111:
      return InitOpolRR(TANK_S);
    case 115:
      return InitFilRR(TANK_K);
    case 116:
      return InitCircRR(TANK_K);
    case 118:
      return InitCheckConc(TANK_K);
    case 119:
      return InitAddRR(TANK_K, step_to_init, not_first_call);
    case 121:
      return InitOpolRR(TANK_K);

    case 151:
      return SCInitPumping(WATER, TANK_W, KANAL, -1, step_to_init,
                           not_first_call);
    case 152:
      return SCInitPumping(WATER, TANK_W, KANAL, -1, step_to_init,
                           not_first_call);
    case 153:
      return SCInitPumping(WATER, WATER, TANK_W_MG, -1, step_to_init,
                           not_first_call);
    case 154:
      return SCInitPumping(WATER, -1, -1, TANK_W_DREN, step_to_init,
                           not_first_call);
    case 155:
      return SCInitPumping(SHCH, TANK_S, TANK_S, -1, step_to_init,
                           not_first_call);
    case 156:
    ...
    case 160:
      return SCInitPumping(SHCH, WATER, TANK_S, -1, step_to_init,
                           not_first_call);
    case 161:
      return SCInitPumping(SHCH, TANK_W, TANK_W_MG, TANK_S_DREN, step_to_init,
                           not_first_call);
    case 162:
      return SCInitPumping(KISL, TANK_K, TANK_K, -1, step_to_init,
                           not_first_call);
    case 163:
    ...
    case 167:
      return SCInitPumping(KISL, WATER, TANK_K_MG, -1, step_to_init,
                           not_first_call);
    case 168:
      return SCInitPumping(KISL, TANK_S, TANK_S_MG, TANK_K_DREN, step_to_init,
                           not_first_call);
    case 169:
    ...
    case 173:
      return SCInitPumping(SHCH, WATER, TANK_W, -1, step_to_init,
                           not_first_call);
    case 174:
      return SCInitPumping(SHCH, TANK_K, TANK_K_MG, TANK_W_DREN, step_to_init,
                           not_first_call);
    case 175:
      return SCInitPumping(KISL, TANK_S, TANK_W_MG, -1, step_to_init,
                           not_first_call);
    case 176:
      return SCInitPumping(KISL, TANK_S, NEUTRO, -1, step_to_init,
                           not_first_call);
    case 177:
      return SCInitPumping(KISL, TANK_S, TANK_W_MG, -1, step_to_init,
                           not_first_call);
    case 178:
      return SCInitPumping(KISL, TANK_S, NEUTRO, -1, step_to_init,
                           not_first_call);
    case 179:
      return SCInitPumping(KISL, WATER, TANK_S_MG, -1, step_to_init,
                           not_first_call);
    case 180:
      return SCInitPumping(KISL, TANK_W, TANK_W_MG, TANK_S_DREN, step_to_init,
                           not_first_call);
    case 181:
      return SCInitPumping(SHCH, TANK_K, NEUTRO, -1, step_to_init,
                           not_first_call);
    case 182:
      return SCInitPumping(SHCH, TANK_K, NEUTRO, -1, step_to_init,
                           not_first_call);
    case 183:
      return SCInitPumping(KISL, TANK_W, NEUTRO, TANK_K_DREN, step_to_init,
                           not_first_call);
    case 184:
      return SCInitPumping(KISL, TANK_W, NEUTRO, TANK_K_DREN, step_to_init,
                           not_first_call);
    case 185:
      return SCInitPumping(WATER, WATER, TANK_S_MG, TANK_KW_DREN, step_to_init,
                           not_first_call);
    case 186:
      return SCInitPumping(WATER, WATER, TANK_K_MG, TANK_SW_DREN, step_to_init,
                           not_first_call);
    case 187:
      return SCInitPumping(WATER, WATER, TANK_W_MG, TANK_SK_DREN, step_to_init,
                           not_first_call);
    case 188:
      return SCInitPumping(-1, -1, -1, TANK_SKW_DREN, step_to_init,
                           not_first_call);

    case 300:
      return InitCustomStep(WATER, WATER, KANAL, 0, step_to_init,
                            not_first_call);
...
int cipline_tech_object::EvalPIDS() {
  ...
  if (!pidf_override) {
    if (dev_ai_pump_frequency) {
      if (dev_ai_pump_feedback) {
        PIDF->eval(dev_ai_pump_feedback->get_value(),
                   dev_ai_pump_frequency->get_value());
      } else {
  ...
  //Клапан пара
  if (ao->get_value() > 1 && PIDP->get_state() == ON &&
      cnt->get_flow() > rt_par_float[P_R_NO_FLOW] && NP->get_state() == ON) {
...
int cipline_tech_object::EvalCipInProgress() {
  ...
  if (res > 0 && curstep != 555) {  // All Ok step is over
    int nowstep = curstep;
    ...
    if (res < 0) {  // was error, time it stopped
      state = res;
      Stop(curstep);
      state = res;
      return res;
...
int cipline_tech_object::_DoStep(int step_to_do) {
  ...
  if (step_to_do == 7 || step_to_do == 8 || step_to_do == 5 || step_to_do == 16)
    is_prerinse = true;

  if (step_to_do == 35 || step_to_do == 37 || step_to_do == 55 ||
      step_to_do == 57)
  ...
  if (dev_upr_prerinse && dev_upr_prerinse == dev_upr_intermediate_rinse &&
      dev_upr_intermediate_rinse == dev_upr_postrinse) {
    ...
    if (dev_upr_intermediate_rinse) {
      if (is_intermediate_rinse)
        dev_upr_intermediate_rinse->on();
      else
        dev_upr_intermediate_rinse->off();
    }
    if (dev_upr_postrinse) {
      if (is_postrinse)
        dev_upr_postrinse->on();
      else
        dev_upr_postrinse->off();
    }
  }
  ...
  if (dev_upr_desinfection) {
    if (step_to_do >= 64 && step_to_do <= 66)
      dev_upr_desinfection->on();
    else
      dev_upr_desinfection->off();
  }
  if (dev_upr_circulation) {
    if ((steps_circulation.count(step_to_do) && circ_temp_reached) ||
        ((steps_additional_rinse.count(step_to_do) ||
          (use_circulation_on_v2_supply &&
           steps_v2_supply.count(step_to_do))) &&
         (!wasflip))) {
  ...
  switch (step_to_do) {
    case 0:
      ...
      return 0;
    case 5:
      return ToObject(tank_w_src, KANAL);
    case 6:
      return OporCIP(KANAL);
    case 7:
      return FromObject(tank_w_src, KANAL);
    case 8:
      return ToObject(tank_w_src, KANAL);
    case 9:
      return OporCIP(KANAL);
    case 10:
      return FillCirc(WITH_RETURN);
    case 11:
      return Circ(WATER);
    case 12:
      return OporCirc(KANAL);
    case 13:
      return FillCirc(WITH_RETURN);
    case 14:
      return ToObject(tank_w_src, KANAL);
    ...
    case 16:
      return OporCirc(KANAL);
    case 22:
      return ToObject(TANK_S, KANAL);
    case 23:
      return OporCIP(KANAL);
    case 24:
      return FromObject(TANK_S, KANAL);
    case 26:
      return FillCirc(WITH_RETURN);
    case 28:
      return Circ(SHCH);
    case 29:
      return OporCirc(TANK_S);

    case 31:
      return FillCirc(WITH_WATER);
    case 33:
      return ToObject(pr_media, TANK_S);
    case 34:
      return OporCIP(TANK_S);
    case 35:
      return FromObject(pr_media, TANK_S);
    case 37:
      return ToObject(pr_media, tank_w_dest);
    case 39:
      return OporCirc(tank_w_dest);
    case 40:
      return OporCIP(tank_w_dest);

    case 42:
      return ToObject(TANK_K, KANAL);
    case 43:
      return OporCIP(KANAL);
    case 44:
      return FromObject(TANK_K, KANAL);
    case 46:
      return FillCirc(WITH_RETURN);
    case 48:
      return Circ(KISL);
    case 49:
      return OporCirc(TANK_K);
    case 53:
      return ToObject(WATER, TANK_K);
    case 54:
      return OporCIP(TANK_K);
    case 55:
      return FromObject(WATER, TANK_K);
    case 57:
      return ToObject(WATER, tank_w_dest);
    case 59:
      return OporCirc(tank_w_dest);
    case 60:
      return OporCIP(tank_w_dest);
    ...
    case 63:
      return OporCIP(tank_w_dest);
    case 64:
      return FromObject(WATER, tank_w_dest);
    case 65:
      return FillCirc(WITH_WATER);
    case 66:
      return Circ(HOT_WATER);
    case 67:
      return OporCirc(tank_w_dest);

    case 71:
      return FillCirc(SANITIZER);
    case 72:
      return ToObject(SANITIZER, tank_w_dest);
    case 73:
      return OporCIP(tank_w_dest);
    case 74:
      return FromObject(SANITIZER, tank_w_dest);
    case 75:
      return FillCirc(SANITIZER);
    case 76:
      return DoseRR(SANITIZER);
    case 77:
      return Circ(SANITIZER);
    case 78:
      return OporCirc(KANAL);
    case 79:
      return OporCIP(KANAL);
    ...
    case 81:
      return FillCirc(WITH_WATER);
    case 83:
      return ToObject(WATER, cleanrinsingto);
    case 84:
      return OporCIP(cleanrinsingto);
    case 85:
      return FromObject(WATER, cleanrinsingto);
    case 86:
      return ToObject(WATER, cleanrinsingto);
    case 91:
      return OporCIP(cleanrinsingto);
    case 105:
      return FilRR(TANK_S);
    case 106:
      return CircRR(TANK_S);
    case 108:
      return CheckConc(TANK_S);
    case 109:
      return AddRR(TANK_S);
    case 111:
      return OpolRR(TANK_S);
    case 115:
      return FilRR(TANK_K);
    case 116:
      return CircRR(TANK_K);
    case 118:
      return CheckConc(TANK_K);
    case 119:
      return AddRR(TANK_K);
    case 121:
      return OpolRR(TANK_K);

    case 151:
      return SCPumping(WATER, TANK_W, KANAL, -1);
    case 152:
      return SCPumping(WATER, TANK_W, KANAL, -1);
    case 153:
      return SCPumping(WATER, WATER, TANK_W_MG, -1);
    case 154:
      return SCPumping(WATER, -1, -1, TANK_W_DREN);
    case 155:
      return SCPumping(SHCH, TANK_S, TANK_S, -1);
    case 156:
      return SCPumping(SHCH, TANK_S, TANK_W_MG, -1);
    case 157:
      return SCPumping(SHCH, TANK_S, TANK_W_MG, -1);
      //	case 158: return SCPumping(SHCH, TANK_S, TANK_W_MG, -1);
      //	case 159: return SCPumping(SHCH, TANK_S, NEUTRO, -1);
    case 160:
      return SCPumping(SHCH, WATER, TANK_S, -1);
    case 161:
      return SCPumping(SHCH, TANK_W, TANK_W_MG, TANK_S_DREN);
    case 162:
      return SCPumping(KISL, TANK_K, TANK_K, -1);
    case 163:
      return SCPumping(KISL, TANK_K, TANK_S_MG, -1);
    case 164:
      return SCPumping(KISL, TANK_K, TANK_S_MG, -1);
      //	case 165: return SCPumping(KISL, TANK_K, TANK_S_MG, -1);
      //	case 166: return SCPumping(KISL, TANK_K, NEUTRO, -1);
    case 167:
      return SCPumping(KISL, WATER, TANK_K_MG, -1);
    case 168:
      return SCPumping(KISL, TANK_S, TANK_S_MG, TANK_K_DREN);
    case 169:
      return SCPumping(SHCH, TANK_W, TANK_K_MG, -1);
    case 170:
      return SCPumping(SHCH, TANK_W, TANK_K_MG, -1);
      //	case 171: return SCPumping(SHCH, TANK_W, TANK_K_MG, -1);
      //	case 172: return SCPumping(SHCH, TANK_W, NEUTRO, -1);
    case 173:
      return SCPumping(SHCH, WATER, TANK_W, -1);
    case 174:
      return SCPumping(SHCH, TANK_K, TANK_K_MG, TANK_W_DREN);
    case 175:
      return SCPumping(KISL, TANK_S, TANK_W_MG, -1);
    case 176:
      return SCPumping(KISL, TANK_S, TANK_W_MG, -1);
      //	case 177: return SCPumping(KISL, TANK_S, TANK_W_MG, -1);
      //	case 178: return SCPumping(KISL, TANK_S, NEUTRO, -1);
    case 179:
      return SCPumping(KISL, WATER, TANK_S_MG, -1);
    case 180:
      return SCPumping(KISL, TANK_W, TANK_W_MG, TANK_S_DREN);
    case 181:
      return SCPumping(SHCH, TANK_K, NEUTRO, -1);
    case 182:
      return SCPumping(SHCH, TANK_K, NEUTRO, -1);
    case 183:
      return SCPumping(KISL, TANK_W, NEUTRO, TANK_K_DREN);
    case 184:
      return SCPumping(KISL, TANK_W, NEUTRO, TANK_K_DREN);
    case 185:
      return SCPumping(WATER, WATER, TANK_S_MG, TANK_KW_DREN);
    case 186:
      return SCPumping(WATER, WATER, TANK_K_MG, TANK_SW_DREN);
    case 187:
      return SCPumping(WATER, WATER, TANK_W_MG, TANK_SK_DREN);
    case 188:
      return SCPumping(-1, -1, -1, TANK_SKW_DREN);

    case 300:
      return EvalCustomStep(WATER, WATER, KANAL, 0);
    ...
    case 555:
      ...
      if (parpar[0][P_END_WASH_DELAY] > 0)
        wash_end_delay = parpar[0][P_END_WASH_DELAY];
...
void cipline_tech_object::ResetErr() { T[TMR_NO_FLOW]->reset(); }
...
int cipline_tech_object::_CheckErr() {
  ...
  if ((!LL->is_active() && (LM->is_active() || LH->is_active())) ||
      (!LM->is_active() && LH->is_active())) {
    if (get_delta_millisec(bachok_lvl_err_delay) > 5000L &&
        !(block_flags &
          (1 << BE_ERR_LEVEL_BACHOK)))  //если ошибка уровня больше 5 секунд
    {
  ...
  if (T[TMR_NO_FLOW]->get_countdown_time() !=
      (rt_par_float[P_TM_R_NO_FLOW] * 1000L)) {
    T[TMR_NO_FLOW]->set_countdown_time(
        (unsigned long)rt_par_float[P_TM_R_NO_FLOW] * 1000L);
  }
  if (NP->get_state() == ON) {
...
int cipline_tech_object::InitFilRR(int where) {
  ...
  //   printf("\n\rfillRR v1M3: %f,pd: %f, kk: %f, kz: %f, ro: %f", v, pd, kk,
  //   kz, ro); par->setParamM(P_ZAD_FLOW, pd/v);
  ...
  T[TMR_OP_TIME]->set_countdown_time(
      (unsigned long)rt_par_float[P_MAX_OPER_TM] * 1000L);
  T[TMR_OP_TIME]->start();
  ...
  if (0 == first_init_flag) {
    ...
    switch (no_liquid_phase) {
      case 0:  //начало первого цикла добавления раствора - запоминаем время
        ...
        no_liquid_is_warning = 0;
        break;
      case 1:  //не первый цикл. сравниваем время с предыдущим. Если совпадают с
               //допуском до 10% выдаем 1 раз ошибку.
        if (fabs(no_liquid_last_time - v / divider * 1000) <
            no_liquid_last_time * 0.1) {
...
int cipline_tech_object::InitOpolRR(int where) {
  ...
  T[TMR_OP_TIME]->set_countdown_time(
      (unsigned long)rt_par_float[P_MAX_OPER_TM] * 1000L);
  T[TMR_OP_TIME]->start();
  T[TMR_CHK_CONC]->set_countdown_time(
      (unsigned long)rt_par_float[P_TM_NO_CONC] * 1000L);
  rt_par_float[P_SUM_OP] = 0;
...
int cipline_tech_object::FilRR(int where) {
  unsigned long tmp;
  rt_par_float[P_OP_TIME_LEFT] =
      (unsigned long)(T[TMR_OP_TIME]->get_work_time() / 1000);
...
int cipline_tech_object::CircRR(int where) {
  ...
  rt_par_float[P_OP_TIME_LEFT] =
      (unsigned long)(T[TMR_OP_TIME]->get_work_time() / 1000L);
  rt_par_float[P_SUM_OP] = cnt->get_quantity();
...
int cipline_tech_object::_InitToObject(int from, int where, int step_to_init,
                                       int f) {
  float v = 1, p = 1, z = 0;
  //	int ot;
  V05->on();
  ...
  if (step_to_init == 5 || step_to_init == 22 || step_to_init == 42 ||
      step_to_init == 62 || step_to_init == 72 ||
      (step_to_init == 83 && rt_par_float[P_PROGRAM] == SPROG_HOTWATER)) {
    enable_ret_pump = 1;
  } else {
    enable_ret_pump = 0;
  }
  ...
  if (/*disable_tank_heating && */ isTank() &&
      (22 == step_to_init || 42 == step_to_init || 62 == step_to_init)) {
  ...
  T[TMR_OP_TIME]->set_countdown_time(
      (unsigned long)rt_par_float[P_MAX_OPER_TM] * 1000);
  T[TMR_OP_TIME]->start();
...
int cipline_tech_object::_InitFromObject(int what, int where, int step_to_init,
                                         int f) {
  float v = 0, p = 0, z = 0;
  //	int ot;
  if (0 == f) {
  ...
  rt_par_float[P_ZAD_PODOGR] = v;
  if (disable_tank_heating && isTank() &&
      (24 == step_to_init || 44 == step_to_init || 64 == step_to_init)) {
    rt_par_float[P_ZAD_PODOGR] = 0;
    PIDP->off();
  } else {
    PIDP->on();
  ...
  T[TMR_OP_TIME]->set_countdown_time(
      (unsigned long)rt_par_float[P_MAX_OPER_TM] * 1000);
  T[TMR_OP_TIME]->start();
  T[TMR_RETURN]->set_countdown_time(
      (unsigned long)rt_par_float[P_TM_NO_FLOW_R] * 1000);
  T[TMR_RETURN]->reset();
  T[TMR_CHK_CONC]->reset();
  if (what == TANK_K || what == TANK_S || where == TANK_S || where == TANK_K) {
    T[TMR_CHK_CONC]->set_countdown_time(
        (unsigned long)rt_par_float[P_TM_NO_CONC] * 1000 *
        2);  //для ошибки "высокая концентрация в возвратной трубе"
  } else {
    T[TMR_CHK_CONC]->set_countdown_time(
        (unsigned long)rt_par_float[P_TM_NO_CONC] *
        1000);  //для ошибки "нет концентрации в возвратной трубе"
  }
  rt_par_float[P_SUM_OP] = 0;
...
int cipline_tech_object::_InitOporCIP(int where, int step_to_init,
                                      int not_first_call) {
  float z = 0;
  opcip = 0;
  if (isLine()) return 0;
  ...
  T[TMR_OP_TIME]->start();
  T[TMR_RETURN]->set_countdown_time(
      (unsigned long)rt_par_float[P_TM_RET_IS_EMPTY] * 1000);
  T[TMR_RETURN]->reset();
  rt_par_float[P_SUM_OP] = 0;
...
int cipline_tech_object::_InitFilCirc(int with_what, int step_to_init, int f) {
  ...
  switch (with_what) {
    ...
    case WITH_RETURN:
      ...
      SetRet(ON);
      if (isTank() &&
          (26 == step_to_init || 46 == step_to_init || 65 == step_to_init)) {
        PIDP->off();
      } else {
        PIDP->on();
      }
...
int cipline_tech_object::_InitOporCirc(int where, int step_to_init,
                                       int not_first_call) {
  V11->off();
  V00->instant_off();
  V01->on();
...
int cipline_tech_object::_InitCirc(int what, int step_to_init,
                                   int not_first_call) {
  if (0 == not_first_call) {
    circ_podp_count = 0;
    circ_was_feed = 0;
  ...
  unsigned long tm = 0;
  ret_circ_flag = 0;
  if ((circ_tank_s && 28 == step_to_init) ||
      (circ_tank_k && 48 == step_to_init)) {
    V01->off();
    V10->off();
  } else {
    V01->on();
  ...
  switch (what) {
    ...
    case SHCH:
      ...
      if (((int)rt_par_float[P_PROGRAM] & ((1 << PRG_K) + (1 << PRG_S))) ==
          ((1 << PRG_K) + (1 << PRG_S))) {
...
int cipline_tech_object::_ToObject(int from, int where) {
  ...
  if (enable_ret_pump) {
    if ((cnt->get_quantity() >=
         rt_par_float[P_VRAB] - rt_par_float[P_RET_STOP]) ||
        (cnt->get_quantity() <= rt_par_float[P_RET_STOP])) {
      SetRet(OFF);
    } else {
      SetRet(ON);
    }
  ...
  switch (from) {
    ...
    case SANITIZER:
      ...
      if (curstep == 62) {
        rt_par_float[STP_USED_HOTWATER] =
            rt_par_float[STP_USED_HOTWATER] + tmp - rt_par_float[STP_LV];
      }
      rt_par_float[STP_LV] = tmp;
      break;
    case TANK_W:
      if (waterTankIsEmpty()) {
        ...
        if (tankempty &&
            get_delta_millisec(tankemptytimer) > toTankSwitchTimeMs) {
          tankempty = 0;
        }
      }
      ...
      if (tankempty) {
        ...
        rt_par_float[STP_WC_INST_WS] =
            rt_par_float[STP_WC_INST_WS] + tmp - rt_par_float[STP_LV];
        rt_par_float[STP_LV] = tmp;
        V04->off();
        V01->on();
        if (!LM->is_active()) V00->on();
        ...
        tmp = cnt->get_quantity();
        rt_par_float[STP_WS] =
            rt_par_float[STP_WS] + tmp - rt_par_float[STP_LV];
        rt_par_float[STP_LV] = tmp;
        V04->on();
        V00->instant_off();
        V01->off();
    ...
    case TANK_K:
      c = GetConc(KISL);
      tmp = cnt->get_quantity();
      rt_par_float[STP_USED_ACID] =
          rt_par_float[STP_USED_ACID] + tmp - rt_par_float[STP_LV];
      rt_par_float[STP_LV] = tmp;
      if (!LKL->is_active()) {
        return ERR_NO_ACID;
      }
    ...
    case TANK_S:
      c = GetConc(SHCH);
      tmp = cnt->get_quantity();
      rt_par_float[STP_USED_CAUSTIC] =
          rt_par_float[STP_USED_CAUSTIC] + tmp - rt_par_float[STP_LV];
      rt_par_float[STP_LV] = tmp;
      if (!LSL->is_active()) {
  ...
  switch (where) {
    case TANK_W:
      ...
      if (tankfull) {
        if (get_delta_millisec(tankfulltimer) > toTankSwitchTimeMs &&
            (!LWH->is_active())) {
          tankfull = 0;
        }
      }
...
int cipline_tech_object::_OporCIP(int where) {
  ...
  if (dev_ls_ret_pump) {
    if (!dev_ls_ret_pump->is_active()) {
      if (get_delta_millisec(ret_pums_ls_timer) >
          (rt_par_float[P_TM_RET_IS_EMPTY] * 1000L)) {
        return 1;
      }
    }
  }
...
int cipline_tech_object::_Circ(int what) {
  ...
  if (TR->get_value() >=
          rt_par_float[P_ZAD_PODOGR] - rt_par_float[P_DELTA_TR] &&
      cnt->get_flow() > rt_par_float[P_R_NO_FLOW]) {
    T[TMR_OP_TIME]->start();
    circ_temp_reached = 1;
  } else {
    T[TMR_OP_TIME]->pause();
    ...
    case KISL:
      ...
      if (circ_podp_k) {
        if (!LM->is_active() && !LL->is_active() && !LH->is_active() &&
            circ_podp_count < circ_podp_max_count) {
          V02->on();
          if (0 == circ_was_feed) {
            circ_max_timer = get_millisec();
          }
        ...
        if (LM->is_active() || LH->is_active() ||
            (circ_was_feed &&
             get_delta_millisec(circ_max_timer) > circMaxFeedTimeMs)) {
          V02->off();
          if (1 == circ_was_feed) {
            circ_podp_count++;
...
int cipline_tech_object::_OporCirc(int where) {
  ...
  if (cnt->get_quantity() >= rt_par_float[P_V_LL_BOT] ||
      T[TMR_OP_TIME]->is_time_up())
    return 1;
  return 0;
}

int cipline_tech_object::_InitDoseRR(int what, int step_to_init,
                                     int not_first_call) {
  switch (what) {
    case SANITIZER:
      RHI();
  ...
  T[TMR_OP_TIME]->set_countdown_time((u_long)rt_par_float[P_MAX_OPER_TM] *
                                     1000L);
  T[TMR_OP_TIME]->start();
  rt_par_float[P_SUM_OP] = 0;
  NP->on();
...
int cipline_tech_object::init_object_devices() {
  ...
  if (G_DEBUG) {
    printf(
        "Circ options:FW=%d,FS=%d,FK=%d,CS=%d,CK=%d,SP=%d,SPM=%d,count=%d\n\r",
        circ_podp_water, circ_podp_s, circ_podp_k, circ_tank_s, circ_tank_k,
        circ_water_no_pump_stop, circ_medium_no_pump_stop, circ_podp_max_count);
  }
  ...
  if (check_DO(dev_upr_intermediate_rinse, P_SIGNAL_INTERMEDIATE_RINSE))
    return -1;
...
int cipline_tech_object::SCInitPumping(int what, int from, int where,
                                       int whatdrainage, int step, int f) {
  V05->off();
...
int cipline_tech_object::SCPumping(int what, int from, int where,
                                   int whatdrainage) {
  switch (what) {
    case SHCH:
...
int cipline_tech_object::timeIsOut() {
  if (T[TMR_OP_TIME]->get_work_time() >= rt_par_float[P_MAX_OPER_TM] * 1000L &&
...
int cipline_tech_object::InitCustomStep(int what, int from, int where, int how,
                                        int step, int f) {
  int luares = 0;
...
int cipline_tech_object::EvalCustomStep(int what, int from, int where,
                                        int how) {
  int luares = 0;
...
void cipline_tech_object::DateToChar(char* buff) {
  tm curdate = get_time();
  sprintf(buff, "%02d-%02d-%04d %02d:%02d", curdate.tm_mday, curdate.tm_mon + 1,
          curdate.tm_year + 1900, curdate.tm_hour, curdate.tm_min);
}
...
int cipline_tech_object::InitOporCIP(int where, int step_to_init,
                                     int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitCirc(int what, int step_to_init,
                                  int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitToObject(int from, int where, int step_to_init,
                                      int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitFromObject(int what, int where, int step_to_init,
                                        int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitFilCirc(int with_what, int step_to_init,
                                     int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitOporCirc(int where, int step_to_init,
                                      int not_first_call) {
  int luares = 0;
...
int cipline_tech_object::InitDoseRR(int what, int step_to_init,
                                    int not_first_call) {
  int luares = 0;
...
MSAPID::MSAPID(run_time_params_float* par, int startpar, int taskpar,
               i_AO_device* ao /*= 0*/, i_AI_device* ai /*= 0*/,
               i_counter* ai2 /*= 0 */, int outmaxrecalcpar /*= 0*/)
    : uk_1(0),
      ek_1(0),
      ek_2(0),
      q0{},
...
void MSAPID::on(int accel /*= 0 */) { pid_on(accel); }
...
float MSAPID::pid_eval(float current_value, int delta_sign /*= 1 */) {
  if (STATE_OFF == state) {
    if (this->is_down_to_inaccel_mode)
      return 100;
    else
      return 0;
  }
...
void MSAPID::set(float new_z) { par[0][pid_par_offset + P_Z] = new_z; }

u_int_4 MSAPID::get_state() const { return state; }
