...
int linux_tcp_client::Communicate(unsigned int bytestosend) {
  ...
  if (tcp_communicator_linux::sendall(socket_number, (unsigned char*)buff,
                                      bytestosend, 0, timeout * 1000, ip,
                                      "tcp client", 0) < 0) {
    Disconnect();
    return 0;
  }

  int res = tcp_communicator_linux::recvtimeout(
      socket_number, (unsigned char*)buff, buff_size, 0, timeout * 1000, ip,
      "tcp client", 0);
...
linux_tcp_client::linux_tcp_client(const char* client_ip,
                                   ...
                                   unsigned char alarm_subclass,
                                   unsigned int exchange_buf_size /*= 256*/,
...
int linux_tcp_client::Connect() {
  ...
  if (socket_number < 0) {
    sprintf(G_LOG->msg,
            "Network device : s%d->\"%s\" error creating socket : timeout (%ld "
            "ms).",
            id, ip, connectTimeout);
  ...
  if (setsockopt(socket_number, SOL_SOCKET, SO_REUSEADDR, &C_ON,
                 sizeof(C_ON))) {
    sprintf(G_LOG->msg,
            "Network device : s%d->\"%s\" error setting socket params : "
            "timeout (%ld ms).",
            id, ip, connectTimeout);
    G_LOG->write_log(i_log::P_ERR);
    close(socket_number);
  ...
  if (res != 0) {
    sprintf(G_LOG->msg,
            "Network device : s%d->\"%s\" error setting nonblock mode : "
            "timeout (%ld ms).",
            id, ip, connectTimeout);
    G_LOG->write_log(i_log::P_ERR);
    close(socket_number);
  ...
  if (res <= 0) {
    sprintf(G_LOG->msg,
            "Network device : s%d->\"%s\" error in connect : timeout (%ld ms).",
            id, ip, connectTimeout);
    G_LOG->write_log(i_log::P_ERR);
    close(socket_number);
    socket_number = 0;
  ...
  if (FD_ISSET(socket_number, &rdevents)) {
    ...
    if (getsockopt(socket_number, SOL_SOCKET, SO_ERROR, &error, &err_len) < 0 ||
        error != 0) {
      sprintf(
          G_LOG->msg,
          "Network device : s%d->\"%s\" error in connect : timeout (%ld ms).",
...
int linux_tcp_client::AsyncConnect() {
  ...
  if (connectedstate == ACS_CONNECTED) {
    ...
    if (connectedstate == ACS_DISCONNECTED) {
      ...
      if (socket_number < 0) {
        sprintf(G_LOG->msg,
                "Network device : s%d->\"%s\" error creating socket : timeout "
                "(%ld ms).",
                id, ip, connectTimeout);
        G_LOG->write_log(i_log::P_ERR);
        return 0;
      ...
      int connectres = connect(socket_number, (struct sockaddr*)&sock_address,
                               sizeof(sock_address));
    ...
    if (connectedstate == ACS_CONNECTING) {
      ...
      if (res == 0) {
        if (get_delta_millisec(async_startconnnect) > timeout) {
          sprintf(G_LOG->msg,
                  "Network device : s%d->\"%s\" disconnected on timeout : "
                  "timeout (%ld ms).",
                  id, ip, connectTimeout);
          G_LOG->write_log(i_log::P_ERR);
      ...
      if (res < 0) {
        sprintf(
            G_LOG->msg,
            "Network device : s%d->\"%s\" connect error : timeout (%ld ms).",
            id, ip, connectTimeout);
        G_LOG->write_log(i_log::P_ERR);
        shutdown(socket_number, SHUT_RDWR);
        close(socket_number);
      ...
      if (FD_ISSET(socket_number, &rdevents)) {
        ...
        if (getsockopt(socket_number, SOL_SOCKET, SO_ERROR, &error, &err_len) <
            ...
            error != 0) {
          sprintf(G_LOG->msg,
                  "Network device : s%d->\"%s\" error in connect(select) : "
                  "timeout (%ld ms).",
                  id, ip, connectTimeout);
          G_LOG->write_log(i_log::P_ERR);
          shutdown(socket_number, SHUT_RDWR);
...
int linux_tcp_client::AsyncSend(unsigned int bytestosend) {
  ...
  int res = tcp_communicator_linux::sendall(socket_number, (unsigned char*)buff,
                                            bytestosend, 0, timeout * 10, ip,
                                            "async tcp client", 0);
  ...
  if (res < 0) {
    ...
    Disconnect();
    sprintf(G_LOG->msg,
            "Network device : s%d->\"%s\" error on send - disconnect : timeout "
            "(%ld ms).",
            socket_number, ip, connectTimeout);
    G_LOG->write_log(i_log::P_ERR);
    return 0;
  } else {
...
int linux_tcp_client::AsyncRecive() {
  ...
  if (get_delta_millisec(asyncReciveTime) >= async_timeout &&
      newDataIsAvailable) {
    newDataIsAvailable = false;
    res = tcp_communicator_linux::recvtimeout(
        socket_number, reinterpret_cast<unsigned char*>(buff), buff_size, 0, 0,
        ip, "tcp client", nullptr);
  }
...
int linux_tcp_client::get_async_result() {
  /// В процессе соединения циклично вызываем функцию для реализации
