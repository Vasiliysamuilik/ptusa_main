...
int tcp_communicator_linux::net_init() {
  ...
  int err = master_socket =
      socket(PF_INET, type, protocol);  // Cоздание мастер-сокета.

  if (G_DEBUG) {
    printf(
        "tcp_communicator_linux:net_init() - master socket created. "
        "Has number %d\n\r",
        master_socket);
  }
  ...
  if (master_socket < 0) {
    ...
    PAC_critical_errors_manager::get_instance()->reset_global_error(
        PAC_critical_errors_manager::AC_NET,
        PAC_critical_errors_manager::AS_SOCKET_F, 0);
  }
  ...
  if (setsockopt(master_socket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) {
    ...
    PAC_critical_errors_manager::get_instance()->reset_global_error(
        PAC_critical_errors_manager::AC_NET,
        PAC_critical_errors_manager::AS_SETSOCKOPT_F, 0);
  }
  ...
  if (err < 0) {
    ...
    PAC_critical_errors_manager::get_instance()->reset_global_error(
        PAC_critical_errors_manager::AC_NET,
        PAC_critical_errors_manager::AS_BIND_F, 0);
  }
  ...
  if (G_DEBUG) {
    printf(
        "tcp_communicator_linux:net_init() - modbus socket created. "
        "Has number %d\n\r",
        modbus_socket);
  }
...
//------------------------------------------------------------------------------
tcp_communicator_linux::~tcp_communicator_linux() { net_terminate(); }
//------------------------------------------------------------------------------
int tcp_communicator_linux::evaluate() {
  ...
  while (count_cycles < max_cycles) {
    ...
    FD_ZERO(&rfds);
    for (u_int i = 0; i < sst.size(); i++) {
      if (sst[i].active && sst[i].is_listener && !sst[i].evaluated) {
    ...
    if (rc < 0) {
      sprintf(G_LOG->msg, "Network communication : select socket : %s.",
              strerror(errno));
    ...
    for (u_int i = 0; i < sst.size(); i++) /* scan all possible sockets */
    {
      ...
      if (FD_ISSET(sst[i].socket, &rfds)) {
        if (sst[i].socket == master_socket || sst[i].socket == modbus_socket) {
          ...
          slave_socket =
              accept(sst[i].socket, (struct sockaddr *)&ssin, &sin_len);

          if (slave_socket <= 0)  // Ошибка.
          {
            sprintf(G_LOG->msg, "Network communication : accept socket : %s.",
                    strerror(errno));
            G_LOG->write_log(i_log::P_ERR);
          ...
          if (fcntl(slave_socket, F_SETFL, O_NONBLOCK) < 0) {
            ...
            sprintf(G_LOG->msg, "Network communication : fcntl socket : %s.",
                    strerror(errno));
            G_LOG->write_log(i_log::P_ERR);
          ...
          if (client) {
            sprintf(G_LOG->msg,
                    "Network communication : accepted %s connection : "
                    "s%d->\"%s\":\"%s\".",
    ...
    for (std::map<int, tcp_client *>::iterator it = clients->begin();
         it != clients->end();) {
      int is_removed = 0;
      if (FD_ISSET(it->second->get_socket(),
                   &rfds))  //если есть событие на сокете
      {
        int err = recvtimeout(
            it->second->get_socket(), (unsigned char *)it->second->buff,
            it->second->buff_size, 1, 0, it->second->ip, "async client", 0);
...
int tcp_communicator_linux::sendall(int sockfd, unsigned char *buf, int len,
                                    int sec, int usec, const char *IP,
                                    const char *name, stat_time *stat) {
  ...
  if (stat) {
    ...
    if (stat->print_cycle_last_h != timeInfo_->tm_hour) {
      ...
      sprintf(G_LOG->msg,
              ...
              sockfd, name, IP, avg_time, stat->min_iteration_cycle_time,
...
//------------------------------------------------------------------------------
int tcp_communicator_linux::recvtimeout(int s, u_char *buf, int len,
...
int tcp_communicator_linux::do_echo(int idx) {
  ...
  if (sock_state.init) {
    ...
    if (sock_state.ismodbus) {
      ...
      err = in_buffer_count =
          recvtimeout(sock_state.socket, buf, BUFSIZE, 0, 50000L,
                      inet_ntoa(sock_state.sin.sin_addr), dev_name,
                      &sock_state.recv_stat, 1);
      if (err == -2) {
        //Если при подключении на модбас-сокет первый раз данные не пришли, то
        //игнорируем(панель Weintek)
        return 0;
      }
    } else {
      // Ожидаем данные с таймаутом 300 мсек.
      err = in_buffer_count = recvtimeout(
          sock_state.socket, buf, BUFSIZE, 0, 300000L,
  ...
  if (net_id == 's' && buf[1] < TC_MAX_SERVICE_NUMBER &&
    ...
    switch (buf[2]) {
      case FRAME_SINGLE:
        ...
        res =
            services[buf[1]]((u_int)(buf[4] * 256 + buf[5]), buf + 6, buf + 5);

        if ((unsigned int)res > max_buffer_use) {
          sprintf(G_LOG->msg,
                  "Network performance : (out) buffer use max = %u, tresh = %u "
                  "(b).",
                  res, BUFSIZE);
          G_LOG->write_log(i_log::P_DEBUG);
      ...
      default:
        ...
        sprintf(G_LOG->msg,
                "tcp_communicator_linux::do_echo wrong command received on "
                "socket %d->\"%s\".",
    ...
    {
      res = services[15]((u_int)(buf[4] * 256 + buf[5]), buf + 6, buf + 6);
      if (res > 0) {
        buf[4] = (res >> 8) & 0xFF;
        buf[5] = res & 0xFF;
      ...
      sprintf(G_LOG->msg, "No such service %d at socket %d->\"%s\".", buf[1],
              sock_state.socket, inet_ntoa(sock_state.sin.sin_addr));
      G_LOG->write_log(i_log::P_WARNING);
    }
  }
