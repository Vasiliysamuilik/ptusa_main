...
TEST(toLuapp, tolua_PAC_dev_open) {
  ...
  EXPECT_EQ(
      0,
      luaL_dostring(
          L, "o1=tech_object( \'O1\', 1, 1, \'O1\', 1, 1, 10, 10, 10, 10 )"));
  EXPECT_EQ(0,
            luaL_dostring(
                L, "o1:get_modes_manager():add_operation(\'Test operation\')"));

  // u_int operation_state::steps_count() const
  EXPECT_EQ(
      0,
      luaL_dostring(
          L, "res=o1:get_modes_manager()[ 1 ][ operation.RUN ]:steps_count()"));
  ...
  EXPECT_EQ(0, luaL_dostring(
                   L, "res=o1:get_modes_manager()[ 1 ][ 1 ][ 1 ]:get_tag()"));
  lua_getfield(L, LUA_GLOBALSINDEX, "res");
  ...
  EXPECT_EQ(0, luaL_dostring(
                   L, "o1:get_modes_manager()[ 1 ][ 1 ][ 1 ]:set_tag( 20 )"));
  EXPECT_EQ(0, luaL_dostring(
                   L, "res=o1:get_modes_manager()[ 1 ][ 1 ][ 1 ]:get_tag()"));
  lua_getfield(L, LUA_GLOBALSINDEX, "res");
  ...
  // int jump_if_action::set_int_property( const char* name, unsigned int idx,
  // int value );
  EXPECT_EQ(0,
            luaL_dostring(
                L,
                "res=o1:get_modes_manager()[ 1 ][ 1 ][ 1 ][ step.A_JUMP_IF ]:"
...
TEST(toLuapp, tolua_PAC_dev_device_param_emulator00) {
  ...
  ASSERT_EQ(0, luaL_dostring(L,
                             "G_DEVICE_MANAGER():add_io_device( 4, 1, \'TE1\', "
                             "\'Test device\', \'\' )"));
  ASSERT_EQ(
      0, luaL_dostring(L, "TE1 = G_DEVICE_MANAGER():get_device( 4, \'TE1\')"));
...
TEST(toLuapp, tolua_PAC_dev_G00) {
  ...
  ASSERT_EQ(0, luaL_dostring(L,
                             "G_DEVICE_MANAGER():add_io_device( "
                             "device.DT_G, device.DST_G_IOL_4, \'G1\', \'Test "
                             "power module\', \'\' )"));
