...
TEST(operation, evaluate_PID) {
  ...
  G_DEVICE_MANAGER()->add_io_device(device::DT_REGULATOR,
                                    device::DST_REGULATOR_PID, "TC1",
                                    "Test controller", "T");
...
TEST(DI_DO_action, check) {
  ...
  EXPECT_EQ(1, res);
  EXPECT_STREQ(EXPECTED_STR.substr(0, SHORT_STR_SIZE - 1).c_str(), msg.c_str());
...
TEST(wash_action, print) {
  ...
  output = testing::internal::GetCapturedStdout();
  EXPECT_EQ(
      "Устройства DI's DO's DEV's R_DEV's AI:"
...
TEST(jump_if_action, is_goto_next_step) {
  ...
  auto action =
      reinterpret_cast<jump_if_action*>((*step)[step::ACTIONS::A_JUMP_IF]);
