...
class base_error {
 public:
  ...
  void reset_errors_params() { err_par[P_PARAM_N] = 0; }
...
class tech_obj_error : public base_error {
 ...
 public:
  ...
  tech_obj_error(tech_object *tech_dev)
      : tech_dev(tech_dev), prev_size(0), was_set_cmd(false) {}
  ...
  unsigned int get_object_n() const { return tech_dev->get_number(); }
...
class errors_manager {
 public:
  ...
  u_int_2 get_errors_id() const { return errors_id; }
...
class siren_lights_manager : public i_Lua_save_device {
 public:
  ...
  void set_red_blink(int type) { is_red_built_in_blink = type; }
  ...
  virtual const char *get_name_in_Lua() const { return "G_SIREN_MNGR"; }
