...
//-----------------------------------------------------------------------------
long device_communicator::write_devices_states_service(long len, u_char* data,
                                                       u_char* outdata) {
  if (len < 1) {
    return 0;
  }
  ...
  switch (data[0]) {
    case CMD_GET_INFO_ON_CONNECT:
      answer_size = sprintf(
          ...
          params_manager::get_instance()
              ->par[0][params_manager::P_IS_RESET_PARAMS],
    ...
    case CMD_GET_DEVICES: {
      ...
      for (u_int i = 0; i < dev.size(); i++) {
        answer_size += dev[i]->save_device((char*)outdata + answer_size);
      }
    ...
    case CMD_EXEC_DEVICE_COMMAND: {
      ...
      int res = lua_manager::get_instance()->exec_Lua_str(
          (char*)data + 1, "CMD_EXEC_DEVICE_COMMAND ");
    ...
    case CMD_GET_PAC_ERRORS: {
      ...
      answer_size +=
          sprintf(str + answer_size, "alarms[ %d ] = \n  {", project_descr_id);
    ...
    case CMD_RESTORE_PARAMS: {
      ...
      int res =
          params_manager::get_instance()->restore_params_from_server_backup(
              (char*)data + 1);
...
void device_communicator::print() const {
  ...
  sprintf(tmp_str, "Device communicator. Dev count = %d.", (int)dev.size());
  print_str(tmp_str, 1);
