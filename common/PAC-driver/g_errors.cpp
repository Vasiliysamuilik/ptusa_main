...
tech_dev_error::tech_dev_error(device *simple_device)
    : base_error(), simple_device(simple_device) {
...
int tech_dev_error::save_as_Lua_str(char *str) {
      ...
      AS_RETURN == error_state)  // Есть ошибка.
  {
    ...
    res += sprintf(str + res, "\tdescription=\"%s - %s\",\n",
                   ...
                   simple_device->get_error_description());
...
int tech_dev_error::set_cmd(int cmd, int object_alarm_number) {
  ...
  switch (cmd) {
    ...
    case C_CMD_ACCEPT:
      if (AS_ALARM == error_state || AS_RETURN == error_state) {
        ...
        if (G_DEBUG) {
          printf(
              "simple_error::set_cmd(...) - error state = %d, \
...
int tech_obj_error::set_cmd(int cmd, int object_alarm_number) {
    ...
    if (errors[i]->n == object_alarm_number) {
      ...
      if (G_DEBUG) {
        printf("Object \"%s\" (%s %d) set error cmd %d to error %d!\n",
               tech_dev->get_name_in_Lua(), tech_dev->get_name(),
               tech_dev->get_number(), cmd, object_alarm_number);
      }
      was_set_cmd = true;
      return 0;
    }
  ...
  if (G_DEBUG) {
    printf(
        "Error! Object \"%s\" (%s %d) set error cmd - alarm number"
        " %d not found!\n",
        tech_dev->get_name_in_Lua(), tech_dev->get_name(),
        tech_dev->get_number(), object_alarm_number);
  }
...
void errors_manager::set_cmd(unsigned int cmd, unsigned int object_type,
  ...
  if (res) {
    ...
    if (G_DEBUG) {
      printf(
          "Error dev_errors_manager::set_cmd(...) - cmd = %u, object_type = %u,\
...
//-----------------------------------------------------------------------------
errors_manager::~errors_manager() { clear(); }
//-----------------------------------------------------------------------------
...
void siren_lights_manager::eval() {
  ...
  if (PAC_critical_errors_manager::get_instance()->get_id() !=
      critical_error_n) {
    critical_error_n = PAC_critical_errors_manager::get_instance()->get_id();
    srn->on();

    if (G_DEBUG) {
      // Print( "PAC_critical_errors_manager::get_error_n() !=
      // critical_error_n\n" );
    }
  }
...
//-----------------------------------------------------------------------------
siren_lights_manager::siren_lights_manager()
    : par(run_time_params_u_int_4(1)),
      green(0),
      red(0),
      yellow(0),
      srn(0),
      is_red_built_in_blink(0),
      critical_error_n(0),
      start_time(0) {
  par[P_MANUAL_MODE] = 0;
