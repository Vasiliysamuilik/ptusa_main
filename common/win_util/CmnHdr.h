...
#define chBEGINTHREADEX(psa, cbStack, pfnStartAddr, pvParam, fdwCreate,     \
                        pdwThreadId)                                        \
...
#ifdef _DEBUG
#define chASSERT(x) \
  if (!(x)) chASSERTFAIL(__FILE__, __LINE__, #x)
#else
#define chASSERT(x)
#endif
