...
#include <windows.h>
//-----------------------------------------------------------------------------
const char* WSA_Last_Err_Decode() { return WSA_Err_Decode(WSAGetLastError()); }
//-----------------------------------------------------------------------------
  ...
  switch (err_code) {
    ...
    case WSAEFAULT:
      return "The name or the namelen argument is not a valid part of the user "
             ...
             "address family. In case WSAStartup - lpWSAData is not a valid "
             "pointer";
    case WSAENOTCONN:
      return "The socket is not connected.";
    case WSAEINPROGRESS:
      return "A blocking Windows Sockets 1.1 call is in progress, or the "
             "service "
             "provider is still processing a callback function.";
    case WSAENETRESET:
      return "The connection has been broken due to the remote host resetting.";
    case WSAEALREADY:
      return "A nonblocking connect call is in progress on the specified "
             "socket. "
    ...
    case WSAESHUTDOWN:
      return "The socket has been shut down; it is not possible to recv on a "
             "socket after shutdown has been invoked with how set to "
             "SD_RECEIVE "
             "or SD_BOTH.";
    case WSAETIMEDOUT:
    ...
    case WSAESOCKTNOSUPPORT:
      return "The specified socket type is not supported in this address "
             "family.";
    case WSAEINVAL:
    ...
    case WSAECONNABORTED:
      return "The virtual circuit was terminated due to a time-out or other "
             ...
             "longer usable.";
    case WSAEISCONN:
      return "The socket is already connected (connection-oriented sockets "
             "only).";
    case WSAENETUNREACH:
      return "The network cannot be reached from this host at this time.";
      // Errors, returned WSAStartup
    case WSASYSNOTREADY:
      return "Underlying network subsystem is not ready for network "
             "communication.";
