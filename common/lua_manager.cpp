...
const char *FILES[FILE_CNT] = {
    "sys.io.lua",        "sys.devices.lua",       "sys.objects.lua",

    "main.io.lua",       "main.objects.lua",      "main.modbus_srv.lua",
    "main.profibus.lua", "main.restrictions.lua",
};
//-----------------------------------------------------------------------------
// I
//Загружаем последовательно Lua-скрипты. Их структура такова, что все
//необходимые действия реализованы в виде функций, которые потом в определенном
//порядке будут выполнятся. Скрипты разделены на системные (общие для всех
//проектов) и проекта (описание отдельного проекта). Системные скрипты имеют
//префикс sys и хранятся в папке "system scripts" на PC. Список скриптов:
...
int lua_manager::init(lua_State *lua_state, const char *script_name,
  ...
  if (dir[0] != '\0' || sys_dir[0] != '\0' || extra_dirs[0] != '\0') {
    G_LOG->notice("path = \"%s\", sys_path = \"%s\", extra_paths = \"%s\"", dir,
                  sys_dir, extra_dirs);
  }
  ...
  for (int i = 0; i < FILE_CNT; i++) {
    ...
    if (FILES_VERSION[i] != res) {
      if (FILES_VERSION[i] < res) {
        G_LOG->critical(
            "File \"%s\" has version %d, must be %d "
  ...
  const int PAC_ID = lua_manager::get_instance()->int_no_param_exec_lua_method(
      "system", "get_PAC_id", "main");
  ...
  const char *PAC_name_rus = G_LUA_MANAGER->char_no_param_exec_lua_method(
      "system", "get_PAC_name_rus", "lua_manager::init");
  if (0 == PAC_name_rus) {
  ...
  const char *PAC_name_eng = G_LUA_MANAGER->char_no_param_exec_lua_method(
      "system", "get_PAC_name_eng", "lua_manager::init");
  if (0 == PAC_name_eng) {
...
int lua_manager::int_2_exec_lua_method(const char *object_name,
                                       const char *function_name, int param1,
                                       int param2,
                                       const char *c_function_name) const {
  int res = 0;
  if (0 ==
      exec_lua_method_var(object_name, function_name, 1, 2, param1, param2)) {
    res = (int)tolua_tonumber(L, -1, 0);
    lua_remove(L, -1);
  } else {
    res = -1;
...
//-----------------------------------------------------------------------------
void *lua_manager::user_object_exec_lua_method(
    const char *object_name, const char *function_name, int param,
    const char *c_function_name) const {
  void *res = NULL;
  if (0 == exec_lua_method(object_name, function_name, param)) {
...
int lua_manager::exec_lua_method(const char *object_name,
  ...
  return exec_lua_method_var(object_name, function_name,
                             is_use_lua_return_value, is_use_param ? 1 : 0,
                             param);
}
//-----------------------------------------------------------------------------
int lua_manager::exec_lua_method_var(const char *object_name,
                                     const char *function_name,
                                     int is_use_lua_return_value, int cnt,
                                     ...) const {
    ...
    if (is_print_stack_traceback) {
      ...
      while (lua_getstack(L, level, &ar) && i_log::C_BUFF_SIZE - res > 0) {
        ...
        res += snprintf(G_LOG->msg + res, i_log::C_BUFF_SIZE - res,
                        "\n\t%s:%d: in function '%s' ('%s')", ar.short_src,
                        ar.currentline, ar.name, ar.namewhat);
        level++;
      }
...
//-----------------------------------------------------------------------------
int lua_manager::int_no_param_exec_lua_method(
...
//-----------------------------------------------------------------------------
const char *lua_manager::char_exec_lua_method(
    const char *object_name, const char *function_name, int param,
    const char *c_function_name) const {
  const char *res = 0;
...
//-----------------------------------------------------------------------------
const char *lua_manager::char_no_param_exec_lua_method(
    const char *object_name, const char *function_name,
    const char *c_function_name) const {
  const char *res = 0;
  if (0 == exec_lua_method(object_name, function_name, 0, 0, 1)) {
...
//-----------------------------------------------------------------------------
lua_State *lua_manager::get_Lua() const { return L; }
//-----------------------------------------------------------------------------
int lua_manager::reload_script(int script_n, const char *script_function_name,
  ...
  if (script_n < SYS_FILE_CNT) {
    sprintf(path, "%s%s", G_PROJECT_MANAGER->sys_path.c_str(), FILES[script_n]);
  } else {
    sprintf(path, "%s%s", G_PROJECT_MANAGER->path.c_str(), FILES[script_n]);
  }
  ...
  //    ); G_LOG->write_log( i_log::P_ERR ); lua_pop( L, 1 ); return 1;
  //    }
  ...
  if (luaL_dofile(L, path) != 0) {
    ...
    if (luaL_dofile(L, FILES[script_n]) != 0) {
      ...
      //    {
      //    sprintf( G_LOG->msg, "\nReload Lua script - %s", lua_tostring( L, -1
      //    ) ); G_LOG->write_log( i_log::P_ERR ); lua_pop( L, 1 );
      //    }
