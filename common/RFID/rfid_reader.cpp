...
extern int G_DEBUG;
//-----------------------------------------------------------------------------
EPC_info::EPC_info(char* EPC_str, int antenna, int RSSI)
...
#ifdef _WIN32
          printf(
              "rfid_reader: Ошибка перевода сокета в "
...
//-----------------------------------------------------------------------------
int rfid_reader::get_EPC_cnt() const { return EPC_cnt; }
//-----------------------------------------------------------------------------
rfid_reader::~rfid_reader() { disconnect(); }
...
//-----------------------------------------------------------------------------
rfid_reader::rfid_reader(const char* ip_address, int number)
    : _hReader(0), number(number) {
  ...
  communicationConfigData.ustCommunicationConfigParameters.stEthernet
      .bIsIPAddress = 0;
  // IPv6 not supported yet
  communicationConfigData.ustCommunicationConfigParameters.stEthernet
      .enIPAddressType = IPAT_IPv4;
  // Note: Let's set our ip address as a network name, it works like a charm.
  // network name, bIsIPAddress must be set to 0
  strcpy(communicationConfigData.ustCommunicationConfigParameters.stEthernet
             .uKindOfConnection.rgcNetworkName,
         this->ip_address);
  // keep-alive time
  communicationConfigData.ustCommunicationConfigParameters.stEthernet
      .udwKeepAliveTime = 3000;
  // NULL: don't use ssh connection, != NULL: password or path to key file
  communicationConfigData.ustCommunicationConfigParameters.stEthernet
      .pcSSHAuth = NULL;
  // NULL: username for ssh connection is "root", != NULL: username for ssh
  // connection
  communicationConfigData.ustCommunicationConfigParameters.stEthernet
      .pcSSHUser = NULL;
}
...
//-----------------------------------------------------------------------------
void rfid_reader::ResultHandlerSetExtendedResultFlag(tResultFlag enResultFlag,
      ...
      while (pResultListEntry != NULL) {
        ...
        if (enComingGoingFlag == CGF_Coming) {
          ...
          for (int j = 0; pResultListEntry->stEPC.ubEPCWordLength > j; j++) {
            sprintf(
                EPC_str + strlen(EPC_str), "%04X",
                pResultListEntry->stEPC
                    .rguwEPC[pResultListEntry->stEPC.ubEPCWordLength - j - 1]);
          }
          ...
          if (G_DEBUG) {
            SetColor(WHITE);
            printf("%d EPC: ", i + 1);
            for (ubTemp = 0; pResultListEntry->stEPC.ubEPCWordLength > ubTemp;
                 ubTemp++)
              printf("%04X",
                     pResultListEntry->stEPC
                         .rguwEPC[pResultListEntry->stEPC.ubEPCWordLength -
                                  ubTemp - 1]);
            printf(", Antenna: %u, RSSI: %u is ", pResultListEntry->ubPort,
                   pResultListEntry->ubRSSI);
            SetColor(GREEN);
            printf("coming\r\n");
            SetColor(WHITE);
          }
          ...
          for (int j = 0; j < r->EPC_cnt; j++) {
            if (pResultListEntry->ubPort == r->EPC_info_array[j].antenna &&
              ...
              for (int l = j; l < r->EPC_cnt - 1; l++) {
                ...
                strcpy(r->EPC_info_array[l].EPC_str,
                       r->EPC_info_array[l + 1].EPC_str);
              }
            ...
            for (ubTemp = 0; pResultListEntry->stEPC.ubEPCWordLength > ubTemp;
              ...
              printf("%04X",
                     pResultListEntry->stEPC
                         .rguwEPC[pResultListEntry->stEPC.ubEPCWordLength -
                                  ubTemp - 1]);
            printf(", Antenna: %u, Reads: %u is ", pResultListEntry->ubPort,
                   pResultListEntry->uwReadCount);
            SetColor(RED);
            printf("going\r\n");
            SetColor(WHITE);
          }
    ...
    if (enReaderErrorCode == REC_NoError) {
...
#endif
      {
        ...
        if (_enResultFlag == RF_NoError || _enResultFlag == RF_NoTag) {
          ...
          if (G_DEBUG) {
            printf(
                "Command \"SyncGetEPCs\" returns with "
                "error %u\r\n",
                _enResultFlag);
          }
        }
      ...
      if (G_DEBUG) {
        printf(
            "Sending the command \"SyncGetEPCs\" was not "
            "successful (%u)\r\n",
            enReaderErrorCode);
      }
    }
  ...
  if (CallSetExtResultFlag(ERF_MASK_A | ERF_MASK_R)) {
    ...
    if (enReaderErrorCode == REC_NoError) {
...
#endif
      {
        ...
        if (_enResultFlag == RF_NoError) {
          ...
          if (G_DEBUG) {
            ...
            SetColor(RED);
            printf(
                "Command \"ASyncReadDataUntilEndOfBankAny\" "
                "exited with error %u\r\n",
                _enResultFlag);
            SetColor(WHITE);
          }
      ...
      if (G_DEBUG) {
        ...
        SetColor(RED);
        printf(
            "Sending the command \"ASyncReadDataUntilEndOfBankAny\" "
...
TBool rfid_reader::CallSetExtResultFlag(TByte ubExtendedResultFlagMask) {
  ...
  ResetEvent(_hResultHandlerEvent);
  tReaderErrorCode enReaderErrorCode =
      SetExtendedResultFlag(_hReader, ubExtendedResultFlagMask);
  if (enReaderErrorCode == REC_NoError) {
    // No error occurs while sending the command to the reader. Let's wait
    // until the result handler was called.
#ifdef _WIN32
...
#endif
    {
      ...
      if (_enResultFlag == RF_NoError) {
        ...
        if (G_DEBUG) {
          printf(
              "Command \"SetExtendedResultFlag\" returns with "
              "error %u\r\n",
              _enResultFlag);
        }
      }
    ...
    if (G_DEBUG) {
      printf(
          "Sending the command \"SetExtendedResultFlag\" was not "
          "successful (%u)\r\n",
          enReaderErrorCode);
    }
